...
def 開始明細完了(利用者ID: str, タスクID: str, 明細SEQ: int, 応答内容: str = "開始処理を完了しました。") -> dict:
    ...
    処理中への切り替え時に AIタスク要求の 応答タイトル・応答内容 はクリアする
    （前回実行分の結果を残さない）。
    """
    初期化()
    conn = 接続取得()
    try:
        ...
        if cur.rowcount <= 0:
            ...
            return _タスク要求取得(conn, 利用者ID, タスクID)
        conn.execute(
            f"UPDATE {AIタスク要求テーブル} SET 状態 = '処理中', 開始日時 = ?, 終了日時 = '', 実行回数 = 1, PID = '', "
            "応答タイトル = '', 応答内容 = '', 更新日時 = ? "
            "WHERE 利用者ID = ? AND タスクID = ? AND 状態 != 'エラー'",
            [now, now, 利用者ID, タスクID],
        )
...
def タイムアウト対象エラー化(対象一覧: list[dict]) -> int:
    ...
    try:
        ...
        for 行 in 対象一覧:
            ...
            if テーブル == AIタスク明細テーブル:
                cur = conn.execute(
                    f"UPDATE {_識別子(テーブル)} SET 状態 = 'エラー', 有効 = 0, PID = '', 更新日時 = ? "
                    "WHERE 利用者ID = ? AND タスクID = ? AND 明細SEQ = ? "
