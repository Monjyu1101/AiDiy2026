...
def _Aチーム改善反映(
    ...
    """タスクIDが目標ループ（PDCA）から投入されたものであれば、Aチーム改善にも反映する。
...
def タスク要求更新登録(
    ...
    画面からの状態変更も Aチーム作業・Aチーム改善へ反映する。反映しないと、目標ループ（PDCA）
    から投入されたタスクを画面で中止したときに Aチーム作業が実行中のまま残り、Aチーム改善が
    ...
    try:
        ...
        # 終了日時は必ず渡す。渡さないと Aチーム作業側が空のまま残り、まだ実行中と見なされる。
        if 状態 == "準備開始":
            # 再準備は最初からやり直すので、チーム側も未終了へ戻す
            _Aチーム作業反映(conn, タスクID, 状態="準備中", 応答内容="", 終了日時="", guard="")
        elif 状態 in ("完了", "済", "エラー", "中止"):
            _Aチーム作業反映(conn, タスクID, 状態=状態, 終了日時=now, guard="")
        else:
            _Aチーム作業反映(conn, タスクID, 状態=状態, 終了日時="", guard="")
        conn.commit()
        return _タスク要求取得(conn, タスクID)
    finally:
        conn.close()
...
def PID全クリア() -> None:
    ...
    目標ループ（PDCA）の空き時間判定がタイムアウトまで通らない。そこで明細に PID が
    あったタスクの要求も、未終了ならここでまとめて終わらせる。

    Aチーム作業から投入されたタスクは、チーム側にもエラーを反映する。反映しないと
    Aチーム作業が実行中のまま、Aチーム改善が未終了のまま残り、目標ループ（PDCA）が
    次の段へ進めなくなる。
    """
    初期化()
...
def タイムアウト対象エラー化(対象一覧: list[dict]) -> int:
    ...
    Aチーム作業から投入されたタスクは、チーム側にもエラーを反映する（PID全クリア と同じ理由。
    反映しないと Aチーム改善が未終了のまま残り、目標ループが次の段へ進めなくなる）。
    """
    if not 対象一覧:
        return 0
    初期化()
