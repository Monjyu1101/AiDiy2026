        ...
        SELECT r.利用者ID, r.タスクID,
         ...
         WHERE {条件}
           ...
           AND m.更新日時 = (
                SELECT MAX(m2.更新日時)
                  FROM {AIタスク明細テーブル} m2
                 WHERE m2.利用者ID = r.利用者ID
                   AND m2.タスクID = r.タスクID
                   AND m2.状態 = '完了'
                   AND m2.応答内容 != ''
           )
...
def 実行待ち明細一覧() -> list[dict]:
    ...
    親の AIタスク要求が 準備完了 / 実行中 のものだけを対象とする
    （準備中・失敗・完了のタスクは実行しない）。
    ...
    try:
        rows = conn.execute(
            ...
            "WHERE m.実行有効 = 1 AND m.状態 = '待機' AND m.PID = '' AND r.状態 IN ('準備完了', '実行中') "
...
def 明細実行開始記録(利用者ID: str, タスクID: str, 明細SEQ: int, pid: int) -> None:
    ...
    try:
        ...
        conn.execute(
            f"UPDATE {AIタスク要求テーブル} SET 状態 = CASE WHEN 状態 = '準備完了' THEN '実行中' ELSE 状態 END, "
...
def 明細完了(利用者ID: str, タスクID: str, 明細SEQ: int, 応答内容: str = "") -> dict:
    ...
    try:
        ...
        cur = conn.execute(
            ...
            "WHERE 利用者ID = ? AND タスクID = ? AND 明細SEQ = ? AND 状態 != 'エラー'",
        ...
        if 残 == 0:
            conn.execute(
                ...
                "WHERE 利用者ID = ? AND タスクID = ? AND 状態 != 'エラー'",
                [now, 応答タイトル, 応答内容, now, 利用者ID, タスクID],
            )
        else:
            conn.execute(
                f"UPDATE {AIタスク要求テーブル} SET 応答タイトル = ?, 応答内容 = ?, 更新日時 = ? "
                "WHERE 利用者ID = ? AND タスクID = ? AND 状態 != 'エラー'",
                [応答タイトル, 応答内容, now, 利用者ID, タスクID],
            )
        conn.commit()
        return _タスク要求取得(conn, 利用者ID, タスクID)
    finally:
...
def 開始明細完了(利用者ID: str, タスクID: str, 明細SEQ: int, 応答内容: str = "開始処理を完了しました。") -> dict:
...
def 終了明細完了(利用者ID: str, タスクID: str, 明細SEQ: int, 応答内容: str = "終了処理を完了しました。") -> dict:
...
def タスク実行有効更新(利用者ID: str, タスクID: str, 実行有効: bool) -> dict:
    ...
    try:
        ...
        for テーブル in (AIタスク要求テーブル, AIタスク明細テーブル):
            conn.execute(
                f"UPDATE {_識別子(テーブル)} SET 実行有効 = ?, 更新日時 = ? WHERE 利用者ID = ? AND タスクID = ?",
...
def 明細実行有効更新(利用者ID: str, タスクID: str, 明細SEQ: int, 実行有効: bool) -> bool:
    ...
    try:
        ...
        cur = conn.execute(
            ...
            "WHERE 利用者ID = ? AND タスクID = ? AND 明細SEQ = ?",
            [1 if 実行有効 else 0, now, 利用者ID, タスクID, 明細SEQ],
        )
        conn.commit()
...
def タイムアウト対象一覧(制限分: int = 30) -> list[dict]:
    ...
    try:
        ...
        結果: list[dict] = []
        for テーブル in (AIタスク要求テーブル, AIタスク明細テーブル):
            明細SEQ列 = "明細SEQ" if テーブル == AIタスク明細テーブル else "'' AS 明細SEQ"
            rows = conn.execute(
                f"SELECT 利用者ID, タスクID, {明細SEQ列}, 状態, PID, 開始日時 "
                f"FROM {_識別子(テーブル)} WHERE {条件}",
                [閾値],
            ).fetchall()
            結果.extend({"テーブル": テーブル, **dict(row)} for row in rows)
        return 結果
...
def タイムアウト対象エラー化(対象一覧: list[dict]) -> int:
    ...
    try:
        ...
        for 行 in 対象一覧:
            ...
            PID = str(行.get("PID", ""))
            開始日時 = str(行.get("開始日時", ""))
            if テーブル not in (AIタスク要求テーブル, AIタスク明細テーブル) or not 利用者ID or not タスクID:
                continue
            if テーブル == AIタスク明細テーブル:
                cur = conn.execute(
                    f"UPDATE {_識別子(テーブル)} SET 状態 = 'エラー', 実行有効 = 0, PID = '', 更新日時 = ? "
                    "WHERE 利用者ID = ? AND タスクID = ? AND 明細SEQ = ? "
                    "AND 状態 != 'エラー' AND 終了日時 = '' AND PID = ? AND 開始日時 = ?",
                    [now, 利用者ID, タスクID, int(行.get("明細SEQ", 0) or 0), PID, 開始日時],
                )
            else:
                cur = conn.execute(
                    f"UPDATE {_識別子(テーブル)} SET 状態 = 'エラー', 実行有効 = 0, PID = '', 更新日時 = ? "
    ...
    try:
        ...
        conn.execute(
            ...
            [
                ...
                要求TASK_AI_NAME, 要求TASK_AI_MODEL, 実行有効値, "準備完了", マーメイド記号,
                "",
                str(仮.get("開始日時", "")),
                _現在日時(),
