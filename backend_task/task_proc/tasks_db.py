    ...
    try:
        ...
        明細状態 = {
            ...
            for row in conn.execute(
                f"SELECT DISTINCT 状態 FROM {AIタスク明細テーブル} WHERE 利用者ID = ? AND タスクID = ?",
                [利用者ID, タスクID],
            )
        }
        ...
        conn.execute(
            ...
            "実行回数 = 0, 更新日時 = ? WHERE 利用者ID = ? AND タスクID = ?",
            [now, 利用者ID, タスクID],
        )
        conn.execute(
...
def 明細再試行(利用者ID: str, タスクID: str, 明細SEQ: int) -> dict:
