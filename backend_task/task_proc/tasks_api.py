...
async def タスク要求更新登録(request: タスク要求更新登録リクエスト) -> dict:
    ...
    ただし実行有効かつ実行条件が 即時（条件なし）の場合は 状態=待機 として更新し、
    実行開始条件の充足待ちをせず即座に実行を開始させる。
    ...
    try:
        ...
        if 状況 == "準備開始":
            ...
            条件辞書 = 実行条件.model_dump() if 実行条件 is not None else (
                tasks_db.実行条件監視取得(利用者ID, タスクID) or {}
            )
            即時実行 = (
                request.実行有効
                and str(条件辞書.get("実行区分", "")) == "即時"
                and str(条件辞書.get("実行条件", "")) == "無し"
            )
            状態 = "待機" if 即時実行 else "準備完了"
            明細再開 = True
        else:
