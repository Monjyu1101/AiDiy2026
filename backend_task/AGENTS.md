...
- DB: `backend_server/_data/AiDiy/database.db` を共有（`Aタスク要求` / `Aタスク明細` / `Aタスク実行条件` テーブル、`Aチーム作業`・`Aチーム状況`も更新）
...
`Aチーム作業`（backend_team）経由で投入された`Aタスク要求`は、タスクIDが一致する`Aチーム作業`行を持ちます。
`sub_start.py` / `sub_proc.py` / `sub_terminate.py` が呼ぶ明細完了系API（`tasks_db.明細完了` / `開始明細完了` /
`終了明細完了` / `明細再試行` / `明細失敗`）は、状態・応答タイトル・応答内容を該当する`Aチーム作業`行にも
同一トランザクションで反映する（`tasks_db._Aチーム作業反映`）。既に`エラー`の`Aチーム作業`行は上書きしない
（`明細失敗`のエラー化と`明細再試行`のエラーからの復帰は例外）。

`Aチーム状況`は、実行開始条件の監視ループ（10秒間隔）の最後に毎回、有効なAチーム要員×実行有効な
Aタスク要求（更新日時が24時間以内）を要員IDで集計して作り直す（`tasks_db.チーム状況更新`）。
