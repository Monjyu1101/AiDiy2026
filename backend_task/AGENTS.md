...
2. 監視ループ（5 秒間隔）が PID 未設定の仮登録を検出し、`sub_init.py` サブプロセスで要求を AI 分解して `Aタスク明細` を生成する。
...
5. 実行開始条件（`Aタスク実行条件`: 時間指定 / 間隔実行 / 定時実行 / フォルダ変化）付きのタスクは本登録後 `準備完了` で待ち、監視ループが毎分（hh:mm 変化時）次回実行日時と条件を確認して、成立時に明細 → 要求の順で `待機` に戻して実行する。対象は要求が `準備完了` / `完了` かつ明細が全件待機または全件完了のタスクのみ（実行途中は開始しない）。即時（条件なし）は本登録で直接 `待機` になる。
