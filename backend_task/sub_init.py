...
2. aidiy_code_agents MCP へタスク分解を依頼し、temp/output/<利用者ID>.<タスクID>.json を出力させる
3. 出力 JSON を検証する
4. 正常時は backend_task へ本登録（仮登録は置き換え）、エラー時は仮登録を『失敗』にする
"""
...
def プロンプト生成(利用者ID: str, タスクID: str, プロジェクト: str, 要求内容: str, 出力JSONパス: str, task_ai_name: str, task_ai_model: str) -> str:
    return f"""次の要求内容を分析してタスク分解し、結果を JSON ファイルとして保存してください。
ファイルの保存先: {出力JSONパス}
...
def 本登録(データ: dict, 行リスト: list[dict]) -> None:
    res = POST送信(f"{TASK_API}/タスク要求/本登録", {
        "利用者ID": 利用者ID,
        "タスクID": タスクID,
        "タイトル": str(データ["タイトル"]).strip(),
        "要求内容": str(データ["要求内容"]).strip(),
...
def main() -> int:
    ...
    try:
        ...
        # 1. code_agents へタスク分解を依頼
        ログ(f"code_agents run 呼び出し (ai={task_ai_name}, model={task_ai_model}, project_path={プロジェクト})")
        payload = {
            "prompt": プロンプト生成(利用者ID, タスクID, プロジェクト, 要求内容, 出力JSONパス, task_ai_name, task_ai_model),
        ...
        ログ(f"code_agents run 応答: {json.dumps(res, ensure_ascii=False)[:300]}")
