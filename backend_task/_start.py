...
軽量な FastAPI 定期タスクサーバーをポート 8093 で起動します。
...
def get_command() -> list[str]:
    python_path = find_python_in_env(BACKEND_TASK_DIR, ENV_CANDIDATES)
...
def check_environment() -> tuple[bool, str]:
    if not BACKEND_TASK_DIR.exists():
        return False, f"フォルダが見つかりません: {BACKEND_TASK_DIR}"
    if not (BACKEND_TASK_DIR / "pyproject.toml").exists():
        return False, f"pyproject.toml が見つかりません: {BACKEND_TASK_DIR / 'pyproject.toml'}"
    python_path = find_python_in_env(BACKEND_TASK_DIR, ENV_CANDIDATES)
    if python_path is not None:
...
def start() -> subprocess.Popen[bytes]:
    return launch_process("バックエンド(task)", get_command(), BACKEND_TASK_DIR)
