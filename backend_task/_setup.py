...
def setup(choices: dict | None = None) -> bool:
    ...
    print_info(f"作業ディレクトリ: {BACKEND_TASK_DIR}")
    print_info("対象: 定期タスク FastAPI (ポート 8093)")

    if not BACKEND_TASK_DIR.exists():
        print_error(f"{label}: フォルダが見つかりません: {BACKEND_TASK_DIR}")
        return False
    if not check_uv_installed():
        ...
        return False
    if not (BACKEND_TASK_DIR / "pyproject.toml").exists():
        print_error(f"{label}: pyproject.toml が見つかりません。")
        return False

    venv_dir = BACKEND_TASK_DIR / BACKEND_TASK_ENV
    if venv_dir.exists():
        print_success(f"{label}: 既存の仮想環境を検出しました: {venv_dir}")

    if not run_command(["uv", "sync", "--upgrade"], cwd=BACKEND_TASK_DIR):
...
def main() -> None:
    global AUTO_MODE
    print_header("バックエンド(task) セットアップ")
    run_setup, AUTO_MODE = ask_start_mode("バックエンド(task) のセットアップを実行しますか?", default="n")
    if not run_setup:
    ...
    if not setup():
        print_error("バックエンド(task) のセットアップに失敗しました。")
        sys.exit(1)
    print_success("バックエンド(task) のセットアップが完了しました。")
