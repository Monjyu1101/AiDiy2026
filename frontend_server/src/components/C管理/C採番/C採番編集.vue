...
const requiredFields = computed(() => ['採番ID', '最終採番値']);
...
const validateField = (field, showMessage = true) => {
  ...
  if (field === '最終採番値' && !/^\d+$/.test(value)) {
    errors[field] = showMessage ? '最終採番値は数値で入力してください。' : 'ERROR';
    return false;
  }
  errors[field] = '';
  return true;
...
// 詳細データ取得
// ==================================================
const loadDetail = async (serialId) => {
  message.value = '';
  try {
    const res = await apiClient.post('/core/C採番/取得', { 採番ID: String(serialId) });
    if (res.data.status === 'OK' && res.data.data) {
      detailData.value = res.data.data;
      applyDataToForm(res.data.data);
    } else {
      showMessage(res.data.message || '採番情報の取得に失敗しました。', 'error');
    }
  } catch (e) {
    showMessage('採番情報の取得でエラーが発生しました。', 'error');
  }
};
...
const applyQueryParams = async (query) => {
  ...
  if (query.モード === '表示' && query.採番ID) {
    mode.value = 'view';
    await loadDetail(query.採番ID);
    return;
  }
...
const deleteData = async () => {
  ...
  const confirmed = await qConfirm(`C採番「${form.採番ID}」を削除しますか？この操作は取り消せません。`);
  if (!confirmed) return;
          ...
          <div class="detail-panel">
            <template v-if="activeTab === 'content'">
              <div class="detail-row row-id">
                <div class="detail-label">採番ID<span class="required-mark">*</span></div>
                  ...
                  <div class="value-column">
                    <div class="input-wrap">
                      <input
                        ...
                        :class="{ 'input-error': errors.採番ID }"
                        @blur="handleBlur('採番ID')"
                        @input="handleInput('採番ID')"
                      />
                      <span v-if="errors.採番ID" class="input-alert">!</span>
                    </div>
                    <div v-if="errors.採番ID && errors.採番ID !== 'ERROR'" class="field-error">{{ errors.採番ID }}</div>
                  </div>
                </div>
              </div>

              <div class="detail-row row-value">
                <div class="detail-label">最終採番値<span class="required-mark">*</span></div>
                <div class="detail-value">
                  <div class="value-column">
                    <div class="input-wrap">
                      <input
                        id="form-serial-value"
                        type="text"
                        v-model="form.最終採番値"
                        inputmode="numeric"
                        pattern="[0-9]*"
                        class="detail-input id-input"
                        :class="{ 'input-error': errors.最終採番値 }"
                        :readonly="isViewMode"
                        @blur="handleBlur('最終採番値')"
                        @input="handleInput('最終採番値')"
                      />
                      <span v-if="errors.最終採番値" class="input-alert">!</span>
                    </div>
                    <div v-if="errors.最終採番値 && errors.最終採番値 !== 'ERROR'" class="field-error">{{ errors.最終採番値 }}</div>
                  </div>
                </div>
              </div>

              <div class="detail-row row-remarks">
                <div class="detail-label">採番備考</div>
                <div class="detail-value">
                  <div class="input-wrap">
                    <textarea
                      id="form-serial-remarks"
                      v-model="form.採番備考"
