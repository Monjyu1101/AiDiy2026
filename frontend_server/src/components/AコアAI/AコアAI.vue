...
const initializeWebSocket = async (既存ソケットID?: string) => {
  try {
    ...
    wsClient.value.on('init', (message) => {
      console.log('[AコアAI] 初期化完了:', message.ソケットID);
      ...
      if (初期データ.モデル設定) {
        ...
        console.log('[AコアAI] モデル設定:', モデル設定.value);
      }
    ...
    // 接続を確立してソケットIDを取得
    console.log('[AコアAI] WebSocket.connect()呼び出し');
    const 取得ソケットID = await wsClient.value.connect();
    ソケットID.value = 取得ソケットID;
    console.log('[AコアAI] WebSocket接続完了 ソケットID:', 取得ソケットID);

    return 取得ソケットID;
  } catch (error) {
    console.error('[AコアAI] WebSocket接続エラー:', error);
...
onMounted(async () => {
  ...
  console.log('[AコアAI] ========================================');
  console.log('[AコアAI] 初期化開始');
  console.log('[AコアAI] URLからのセッションID:', URLのセッションID);
  console.log('[AコアAI] ========================================');
  ...
  setTimeout(() => {
    console.log('[AコアAI] ビジュアライザー要素を検索中...');
    ...
    console.log('[AコアAI] ビジュアライザー要素:', { audioBarsElement, overlayElement });

    if (audioProcessor && audioBarsElement && overlayElement) {
      audioProcessor.setupAudioVisualizer(audioBarsElement, overlayElement);
      console.log('[AコアAI] ビジュアライザーセットアップ完了');
      syncVisualizerVisibility();
    } else {
      console.error('[AコアAI] ビジュアライザー要素が見つかりません', {
  ...
  try {
    ...
    const response = await apiClient.post('/core/AコアAI/初期化', {
      ソケットID: URLのセッションID || '',
      セッションID: URLのセッションID || ''
    });
    ...
    console.log('[AコアAI] WebSocket接続を開始...');
    const 確立済みソケットID = await initializeWebSocket(ソケットID.value || URLのセッションID);
    console.log('[AコアAI] ========================================');
    console.log('[AコアAI] ✅ 初期化完了');
    console.log('[AコアAI] ソケットID:', 確立済みソケットID);
    console.log('[AコアAI] wsConnected:', wsConnected.value);
    console.log('[AコアAI] ========================================');
  } catch (error) {
    console.error('[AコアAI] ========================================');
    console.error('[AコアAI] ❌ 初期化エラー:', error);
    console.error('[AコアAI] ========================================');
...
onBeforeUnmount(() => {
  ...
  if (wsClient.value) {
    console.log('[AコアAI] WebSocket接続を切断します');
...
const saveState = async () => {
  ...
  try {
    if (wsClient.value && wsConnected.value) {
      ...
      console.log('[AコアAI] 状態更新 (WebSocket):', { ボタン });
    }
  } catch (error) {
    console.error('[AコアAI] 状態保存エラー:', error);
  }
};
...
watch(enableSpeaker, (newValue, oldValue) => {
  ...
  if (!newValue && oldValue) {
    ...
    console.log('[AコアAI] スピーカーOFF - 音声クリア');
