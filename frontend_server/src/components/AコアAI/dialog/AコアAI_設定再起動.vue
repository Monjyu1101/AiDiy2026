...
const loadConfig = async () => {
  ...
  try {
    ...
    const response = await apiClient.post('/core/AコアAI/モデル情報/取得', {
      ソケットID
    });
...
const handleSave = async () => {
  ...
  try {
    ...
    const response = await apiClient.post('/core/AコアAI/モデル情報/設定', {
      ソケットID,
      モデル設定: nextSettings,
      再起動要求: {
        reboot1: false,
...
<template>
  ...
  <RebootDialog :show="showRebootDialog" :wait-seconds="30" />
</template>
