...
| `backend_mcp/` | Chrome DevTools、Desktop Capture、SQLite、PostgreSQL、Logs、Code Check、Backup Check、Backup Save、Image Generation、Speech-to-Text、Text-to-Speech の MCP |
...
`backend_mcp` は 14 個の MCP サーバーを同居させます。
...
| `aidiy_image_generation` | AI 画像生成（OpenAI / Gemini / FreeAI） |
...
各 MCP は **SSE（MCP標準）**、**stdio gateway（`mcp_stdio.py`）**、**HTTP POST（FastAPI `/docs`）** の 3 インターフェースを同一ポートで提供します。Python の `requests` でそのまま呼び出せるため、自動化スクリプトやバックエンドルーターからも利用できます。

MCP の使い分けと設定は `.aidiy/knowledge/backend_server,backend_mcp,MCP活用手順.md` を参照してください。
