...
| `backend_local/local_main.py` | ローカル LLM（OpenAI 互換 Gemma 推論） | 8094 |
...
`core_main.py` と `apps_main.py` は同じ SQLite DB を共有します（`backend_task` も同じ DB の `AIタスク要求` / `AIタスク明細` を使用）。
フロントエンドは Vite proxy で `/core/*` を 8091、`/apps/*` を 9098、`/task/*` を 8093 へ振り分けます。
...
| `backend_tools/` | 18 個の MCP（Chrome DevTools、Desktop Capture、SQLite、PostgreSQL、Logs、Code Check、Backup、Image / Movie Generation、Speech-to-Text、Text-to-Speech、OBS Studio Control、FFmpeg Control、Notification Sounds、Code Agents、Chat LLM、Task Agents、Windows Control）と OpenAI / Ollama 互換 Chat Completions |
...
| `A` | AI / Advanced | `A会話履歴`、`AIコア`、`AIタスク要求`、`AIタスク明細` |
...
- `A会話履歴`: 会話履歴保存。
- `AIタスク要求` / `AIタスク明細`: AIタスク自動実行（backend_task が管理、Web / Avatar の AIタスク画面から操作）。明細は `先行SEQ` による DAG で、直列だけでなく水平の並行分岐を含む自由なタスクフローを定義でき、フロー図はクリティカルパス基準で表示する。
...
`backend_tools` は 18 個の MCP サーバーを同居させます。
