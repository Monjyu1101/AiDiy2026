...
### `.aidiy/knowledge` フォルダの使い方（重要）

`.aidiy/knowledge` フォルダは、**コードエージェントの実行ルート直下に置く、そのプロジェクト専用の知見フォルダ**です。
グローバル共通知識ではなく、**プロジェクトごとに分離された修正知見・履歴**として扱ってください。

#### 基本方針
- `.aidiy/knowledge` は **コードエージェントの実行ルート配下**に作成・参照する
- 知見は **そのプロジェクト専用** とし、他プロジェクトと混在させない
- ファイル操作を伴う修正を行う場合、**まず `.aidiy/knowledge/_index.md` を確認**し、類似修正の知見があれば利用する
- 修正と検証が完了したら、必要に応じて `.aidiy/knowledge` 配下へ知見を追記し、次回以降の改造精度を上げる
...
- `.aidiy/knowledge` 更新時は、**アプリ本体の仕様変更と混同しない**こと
  - 本体修正は通常のソースコードへ
  - 知見整理は `.aidiy/knowledge` へ

#### コードエージェント利用時のルール
プロジェクト内のファイル操作を行う場合は、
- `.aidiy/knowledge` フォルダ
- `.aidiy/knowledge/_index.md`
...
**コードエージェント補足（重要）:**
- コードエージェントは project ごとの `.aidiy/knowledge` 知見フォルダを利用する
- AIコアのコードエージェント枠は現行 **6枠**（`code1`〜`code6` / `CODE_AI1_NAME`〜`CODE_AI6_NAME`）で統一する
- 修正系処理では `.aidiy/knowledge/_index.md` の類似知見を参照し、完了後は `.aidiy/knowledge` へ知見整理を追記する
...
- `login` - ログイン画面
- `core` - メインアバター＋AI操作
- `chat` / `file` / `image` / `code1`-`code6` - 各パネル（コード支援パネルは6枠）
...
- `CODE_AI1_NAME`〜`CODE_AI6_NAME`（原則 `_sdk` または `_cli`、例外 `aidiy_hermes`） — コード支援 AI（コードパネル専用）。有効値: `claude_sdk`, `claude_cli`, `copilot_cli`, `codex_cli`, `gemini_cli`, `opencode_cli`, `aidiy_hermes`

音声設定は frontend_avatar の `settings` ウィンドウ（AI設定ダイアログ）から切り替え可能。設定変更後はバックエンドが自動再起動（Reboot機構）。

**音声対話で開発（AI Coding with Voice）：**  
AIコアに接続したまま音声で指示を出してコーディング作業を進める使い方を想定している。コードパネル（code1〜code6）にはコード変更内容が表示され、ファイルパネルでリポジトリのファイルツリーと内容を確認しながら会話できる。バックエンドの `files API`（`/core/files/内容取得`、`/core/files/内容更新`）を通じて AI がファイルを直接読み書きする。
...
- ウィンドウ role 制御（`login` / `core` / `chat` / `file` / `image` / `code1-6` / `settings`）
