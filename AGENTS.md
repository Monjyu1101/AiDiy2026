...
| アクセス URL | Electron 内 http://127.0.0.1:8099 | http://localhost:8099/AiDiy |
...
- `CODE_AI*_NAME`（`_code` サフィックス） — コード支援 AI（コードパネル専用）
...
- `V` = View/Joined data endpoints（DBのVIEWオブジェクトではなく、生SQLクエリ実装）
  - V利用者, V車両, V商品, V配車区分, V生産区分, V生産工程, V商品分類, V商品構成
  - V配車, V生産, V生産払出, V商品入庫, V商品出庫, V商品棚卸, V商品推移表
...
- **Web モード**: 通常ブラウザで http://localhost:8099/AiDiy にアクセス
...
- フロントエンド Avatar (Web ブラウザ): http://localhost:8099/AiDiy ※通常ブラウザからもアクセス可能
