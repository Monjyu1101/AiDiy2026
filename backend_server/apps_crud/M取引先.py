...
def init_M取引先_data(db: Session, 認証情報: Optional[Dict] = None):
    ...
    初期データID = {取引先ID for 取引先ID, *_ in 初期データ}
    旧サンプルID = {'S001', 'C001', 'P001'}
    audit = create_audit_fields(認証情報, is_update=True)

    for 取引先ID, 取引先名, 取引先分類ID, 郵便番号, 住所, 電話番号, メールアドレス, 取引先備考 in 初期データ:
        existing = get_M取引先(db, 取引先ID)
        if existing:
            existing.取引先名 = 取引先名
            existing.取引先分類ID = 取引先分類ID
            existing.取引先郵便番号 = 郵便番号
            existing.取引先住所 = 住所
            existing.取引先電話番号 = 電話番号
            existing.取引先メールアドレス = メールアドレス
            existing.取引先備考 = 取引先備考
            existing.有効 = True
