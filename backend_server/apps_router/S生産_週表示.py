...
def get_weekly_processes(
    ...
    logger.info("S生産_週表示 生産工程一覧取得: 件数=%d", len(items))
...
def update_schedule_drag(
    ...
    logger.info("S生産_週表示 ドラッグ更新開始: 生産伝票ID=%s 生産工程ID=%s", request.生産伝票ID, request.生産工程ID)
    ...
    if request.変更後開始日時 and request.変更後終了日時:
        ...
        if not target_date:
            return schemas.ResponseBase(
                status="NG",
                message="必須パラメータが不足しています（生産伝票ID, 生産工程ID, 日付）",
