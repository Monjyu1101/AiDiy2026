...
@router.post("/トークン更新", response_model=schemas.ResponseBase)
def トークン更新(現在利用者: C利用者 = Depends(deps.get_現在利用者)):
    """操作中とみなしてトークンを再発行する（セッション延長）"""
    token_data = issue_token(現在利用者)
    return schemas.ResponseBase(
        status="OK",
        message="トークンを更新しました",
        data=token_data
