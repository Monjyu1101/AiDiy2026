...
async def モデル情報設定(http_request: Request, request: モデル設定リクエスト):
    ...
    try:
        ...
        # save=True の場合は AiDiy_key.json へ書き込む
        if save and 許可設定:
            try:
                conf = getattr(http_request.app, "conf", None)
                if conf and hasattr(conf, "update"):
                    conf.update(許可設定)
                    return {
                        "status": "OK",
        ...
        # 再起動要求があればバックアップ実行とフラグファイルを作成
            try:
                ...
                reboot_mcp  = bool(再起動要求.get("reboot_mcp"))
                if reboot_core or reboot_apps or reboot_mcp:
                ...
                if reboot_apps:
                    ...
                    with open(os.path.join(バックエンドディレクトリ, "temp", "reboot_apps.txt"), "w", encoding="utf-8") as f:
                        f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                if reboot_mcp:
                    with open(os.path.join(プロジェクトルート, "backend_tools", "temp", "reboot_mcp.txt"), "w", encoding="utf-8") as f:
                        f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                if reboot_core or reboot_apps or reboot_mcp:
                    logger.info(f"再起動要求受信: reboot_core={reboot_core} reboot_apps={reboot_apps} reboot_mcp={reboot_mcp}")
            except Exception:
