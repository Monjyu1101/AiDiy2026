...
- MCP サーバー（Chrome DevTools / Desktop Capture / SQLite / PostgreSQL / Logs / Code Check） → `backend_mcp/AGENTS.md`
...
- 本体の修正・検証・通知が終わったあとで、**同じエージェントをもう一度呼び出して** `.aidiy` へ知見を書き込ませる
- これにより、フロントエンドは **本体修正完了を先に認識** できる
- 自己改善フェーズでは、**アプリ本体の追加修正は行わず**、 `.aidiy` 配下の知見整理だけを行う
...
#### 書き込み先
- `.aidiy` は **コードエージェントの実行ルート直下**を使用する
...
- `.aidiy/_index.md` が存在する場合のみ、プロンプトへ知見参照指示を差し込む
- 差し込み位置は **添付ファイル情報の前** とする
- 追加する指示の意図:
  - `.aidiy` と `_index.md` を確認し
  - 類似修正の知見があれば利用する

#### 実装ルール
- 自己改善処理は **本体修正完了後の別フェーズ** として扱う
- 自己改善処理のために、フロントの完了通知を遅らせない
- `.aidiy` が未作成なら、必要に応じて作成してよい
- `.aidiy` 更新は、単なる作業ログではなく **次回再利用できる知見** を残すことを目的とする
...
- **apps_main.py** (port 8092) - Application features (M系, T系, V系, S系)
  - 管理するテーブル: `M配車区分`, `M生産区分`, `M生産工程`, `M商品分類`, `M車両`, `M商品`, `M商品構成`, `T配車`, `T生産`, `T商品出庫`, `T商品棚卸`, `T商品入庫`
  - 登録するルーター: `M配車区分`, `V配車区分`, `M生産区分`, `V生産区分`, `M生産工程`, `V生産工程`, `M商品分類`, `V商品分類`, `M車両`, `V車両`, `M商品`, `V商品`, `M商品構成`, `V商品構成`, `T配車`, `V配車`, `T生産`, `V生産`, `V生産払出`, `T商品出庫`, `V商品出庫`, `T商品棚卸`, `V商品棚卸`, `T商品入庫`, `V商品入庫`, `V商品推移表`, `S配車_週表示`, `S配車_日表示`, `S生産_週表示`, `S生産_日表示`
  - `@app.on_event("startup")`:
...
- **M配車区分.py**, **M生産区分.py**, **M生産工程.py**, **M商品分類.py**, **M車両.py**, **M商品.py** - Master tables
...
- **__init__.py** - CRUD modules と utilities をエクスポート
  - M系（M配車区分, M生産区分, M生産工程, M商品分類, M車両, M商品, M商品構成）の CRUD をエクスポート
...
- **M配車区分.py**, **M生産区分.py**, **M生産工程.py**, **M商品分類.py**, **M車両.py**, **M商品.py** - Master CRUD
...
- **M配車区分.py**, **M生産区分.py**, **M生産工程.py**, **M商品分類.py**, **M車両.py**, **M商品.py** - M系テーブルCRUDエンドポイント
  - 各テーブルで以下のエンドポイントを提供:
...
- **V配車区分.py**, **V生産区分.py**, **V生産工程.py**, **V商品分類.py**, **V車両.py**, **V商品.py**, **V商品構成.py**, **V配車.py** - V系 VIEW query endpoints
  - 各VIEWで以下のエンドポイントを提供:
