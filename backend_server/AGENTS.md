...
- **AI統合** (Anthropic Claude, OpenAI, Google Gemini, Ollama)
...
**9. AI統合機能 (AIコア/):**
- マルチベンダーAI対応: Anthropic Claude, OpenAI, Google Gemini, Ollama
- Ollama チャット: `ollama_chat` が Ollama（ローカル/クラウド）に対応
- freeai_chat: `freeai_key_id` を利用して Gemini にアクセス
- ストリーミング処理: `AIストリーミング処理.py`
- 音声処理: `AI音声処理.py`, `AI音声認識.py`
- チャット: `AIチャット.py`, `AIチャット_gemini.py`, `AIチャット_openrt.py`
- コード生成: `AIコード.py`, `AIコード_cli.py`, `AIコード_claude.py`

### 新しい Code CLI を追加する手順（重要）

`aidiy_hermes` 統合時の実装を、今後の CLI 追加手順として残します。
...
- 既存実装の `aidiy_hermes` のように、専用コマンド名・実行ファイル探索・モデル引数の付け方が CLI ごとに異なる点を吸収する
...
- 固定モデル一覧やローカル上書きが必要な場合は `_sync_local_model_configs()` に `AiDiy_code_<cli名>.json` の読込を追加
- `get_code_models()` の返却値に新CLIを追加
- `aidiy_hermes` のように既存 provider 一覧から動的生成する方式も可

#### 4. 設定JSONの追加
対象:
- `conf/conf_json.py`
- `_config/AiDiy_key.json`
- `_config/AiDiy_code_<cli名>.json`（必要な場合）

- `conf_json.DEFAULT_CONFIG` に `CODE_<CLI名>_MODEL` を追加
- `AiDiy_key.json` にも同じキーを追加
- 固定モデル一覧を持つ方式なら `AiDiy_code_<cli名>.json` を作成し、最低でも `auto` モデルを定義する
...
- 固定モデル一覧を持つ方式なら `_config/AiDiy_code_<cli名>.json` が存在する
- バージョン確認が通る
- 新規会話コマンドが組み立てられる
- 継続会話コマンドが組み立てられる
- Windows 特有の呼び出し条件がある場合、その分岐が入っている

#### 7. `aidiy_hermes` 統合時の具体例
- CLI名: `aidiy_hermes`
- バージョン確認:
  - Windows/非Windows: `aidiy_hermes --version`
- 新規会話:
  - `aidiy_hermes -Q -z "..."`
- 継続会話:
  - 現行実装では専用の `--continue` は使わず、会話履歴を AIコード側で保持したうえで同じ `aidiy_hermes -Q -z "..."` 形式で再実行
- モデル設定キー:
  - `CODE_AIDIY_HERMES_MODEL`
- モデル定義:
  - `conf/conf_model.py` の `get_code_models()` で `aidiy_hermes` を返す
  - モデル一覧は `_get_aidiy_hermes_models()` が Ollama モデル一覧から動的生成するため、専用の `AiDiy_code_*.json` は持たない

#### 8. `opencode_cli` 統合時の具体例
- CLI名: `opencode_cli`
- バージョン確認:
  - Windows: `%USERPROFILE%\.local\bin\opencode.exe --version` または npm の `opencode.cmd --version`
  - 非Windows: `opencode --version`
- 新規会話:
  - `opencode run "プロンプト"`
- 継続会話（`-c` フラグ）:
  - `opencode run "プロンプト" -c`
- モデル設定キー:
  - `CODE_OPENCODE_CLI_MODEL`
- モデル定義:
  - `conf/conf_model.py` の `get_code_models()` で `opencode_cli` を返す
  - 固定モデル一覧は `_config/AiDiy_code_opencode_cli.json` で定義。最低 `auto` エントリが必要
...
- **_config/** - 設定ファイル（`.gitignore` 対象。実ファイル内容は同期しない。必要な場合は `*.example.json` / `*.template.json` のみ共有）
  ...
  - `AiDiy_code_opencode_cli.json` - opencode CLI モード設定（固定モデル一覧用。最低 `auto` エントリが必要）
  - `aidiy_hermes` は専用の `AiDiy_code_*.json` を持たず、`CODE_AIDIY_HERMES_MODEL` と `conf_model.py` の動的モデル生成で扱う
  - `opencode_cli` は `AiDiy_code_*.json` で固定モデル一覧を定義。`auto` エントリのみでも動作可能
...
- プロパティ:
  ...
  - `ボタン状態`: ボタンの状態（スピーカー、マイク、カメラ、チャット、エージェント1〜6、チャットモード）
...
- `CODE_AI1_NAME`〜`CODE_AI6_NAME`: `claude_sdk` / `claude_cli` / `copilot_cli` / `codex_cli` / `gemini_cli` / `opencode_cli` / `aidiy_hermes`（原則 `_sdk` または `_cli`、例外 `aidiy_hermes`）
...
- **opencode CLI** (`opencode_cli`)
  - モデル設定: `CODE_OPENCODE_CLI_MODEL`
  - コマンド: `opencode` (npm グローバル or `.local/bin/opencode.exe`)
  - 会話継続: `-c` フラグで対応（`opencode run "..." -c`）
  - 実行ファイル探索: Windows では `%USERPROFILE%\.local\bin\opencode.exe`、npm の `opencode.cmd` の順に探索

- **AiDiy Hermes** (`aidiy_hermes`)
  - モデル設定: `CODE_AIDIY_HERMES_MODEL`
  - コマンド: `aidiy_hermes`
  - モデル一覧: Ollama モデル一覧から動的生成（`conf/conf_model.py::_get_aidiy_hermes_models()`）
  - 実体: `backend_hermes/cli_main.py`
