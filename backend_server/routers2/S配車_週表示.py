...
from crud2.utils import update_audit_fields
from log_config import get_logger

logger = get_logger(__name__)

router = APIRouter(prefix="/apps/S配車_週表示", tags=["S配車_週表示"])
...
def get_weekly_vehicles(
    ...
    logger.info("S配車_週表示 車両一覧取得: 件数=%d", len(items))
...
def get_weekly_schedules(
    ...
    logger.info("S配車_週表示 配車一覧取得: 開始=%s 終了=%s 件数=%d", start_date, end_date, len(items))
    return schemas.ResponseBase(
        status="OK",
        message="配車一覧を取得しました",
        data={"配車一覧": items}
...
def get_weekly_view_data(
    ...
    vehicles_response = get_weekly_vehicles(db=db, 現在利用者=現在利用者)
...
def update_schedule_drag(
    ...
    if request.変更後開始日時 and request.変更後終了日時:
        ...
        if not target_date:
            return schemas.ResponseBase(
                status="NG",
                message="必須パラメータが不足しています（配車伝票ID, 車両ID, 日付）",
    ...
    logger.info("S配車_週表示 ドラッグ更新完了: 配車伝票ID=%s", request.配車伝票ID)
    return schemas.ResponseBase(
        status="OK",
...
def update_schedule_resize(
    ...
    logger.info("S配車_週表示 リサイズ更新完了: 配車伝票ID=%s", request.配車伝票ID)
    return schemas.ResponseBase(
        status="OK",
        message="配車期間を更新しました",
        data={"配車伝票ID": request.配車伝票ID}
...
def get_display_data_last_modified(
    ...
    if not request.開始日付 or not request.終了日付:
        logger.warning("S配車_週表示 最終更新日時: 日付範囲不足")
        return schemas.ResponseBase(
            status="NG",
    ...
    logger.info("S配車_週表示 最終更新日時取得: 開始=%s 終了=%s 結果=%s", request.開始日付, request.終了日付, 最終更新日時)
    return schemas.ResponseBase(
        status="OK",
