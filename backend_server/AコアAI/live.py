...
旧実装（RiKi_AiDiy_AコアAI__server_live.py）と同じ手順で実装
"""
...
class Live:
    ...
    def _select_ai_module(self):
        ...
        module_name = "AコアAI.live_gemini"
        ai_name = (self.AI_NAME or "").lower()
        if ai_name == "openai_live":
            module_name = "AコアAI.live_openai"
        try:
    ...
    async def _ensure_ai_instance(self):
        ...
        logger.info(f"[Live] LiveAI初期化開始: {self.AI_NAME} (AコアAIセッション: {self.ソケットID})")
        try:
            api_key = ""
            organization = ""
            ai_name = (self.AI_NAME or "").lower()
            ...
            logger.info(f"[Live] LiveAI初期化完了: {self.AI_NAME} (AコアAIセッション: {self.ソケットID})")
            return self.AIインスタンス
        except Exception as e:
