...
try:
    from base.hermes_logging import setup_logging as _setup_logging
...
try:
    ...
    from base.hermes_constants import apply_ipv4_preference as _apply_ipv4
...
from base.hermes_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def cmd_chat(args):
    ...
    # Import and run the AiDiy CLI entry point. The old Hermes CLI accepted a
    # kwargs-based ``cli.main`` function; AiDiy Hermes exposes argparse-style
    # ``cli_main.main(argv)`` instead, so translate the common options here.
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from core.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        from core.bedrock_adapter import (
            has_aws_credentials,
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from core.gemini_native_adapter import probe_gemini_tier
        except Exception:
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from base.hermes_constants import display_hermes_home as _dhh_fn
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            import importlib
            import base.hermes_constants as _hc
        ...
        try:
            ...
            try:
                from base.hermes_constants import (
...
""",
    ...
    def cmd_acp(args):
        ...
        try:
#             from acp_adapter.entry import main as acp_main  # removed: not in aidiy
...
""",
    ...
    if (
        ...
        try:
            from hermes_cli.config import load_config
            from core.shell_hooks import register_from_config
