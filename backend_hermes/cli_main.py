...
Usage:
    python cli_main.py                          # Start interactive mode with all tools
    python cli_main.py --toolsets web,terminal  # Start with specific toolsets
    python cli_main.py --skills hermes-agent-dev,github-auth
    python cli_main.py -q "your question"       # Single query mode
    python cli_main.py --list-tools             # List available tools and exit
"""
...
# Upstream modules import the agent package as `agent.*`; this tree keeps that
# code under `backend_hermes/core`.
...
# The AiDiy Python-only build does not carry the upstream cron package.
try:
...
def _cprint(text: str):
    ...
    """
    def _plain_print() -> None:
        # Non-interactive Windows subprocesses may not have a console screen
        # buffer, so prompt_toolkit output construction can fail before it
        # writes anything. Keep text-mode automation usable.
        print(text, file=sys.stderr if _CPRINT_TO_STDERR else sys.stdout)

    if _CPRINT_TO_STDERR:
        _plain_print()
        return
...
@contextmanager
def _route_cprint_to_stderr():
    global _CPRINT_TO_STDERR
    previous = _CPRINT_TO_STDERR
    _CPRINT_TO_STDERR = True
    try:
        yield
    finally:
        _CPRINT_TO_STDERR = previous
...
def _load_aidiy_config() -> dict:
    try:
        if _AIDIY_KEY_JSON.exists():
            with _AIDIY_KEY_JSON.open("r", encoding="utf-8-sig") as f:
                return json.load(f)
    except Exception:
        pass
    return {}
...
class HermesCLI:
    ...
    def __init__(
        ...
        self._aidiy_provider_slug: Optional[str] = None
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        # CLI providers use a virtual "auto" model — show the provider name
        # instead so the status bar identifies which CLI is in use.
        if model_short.lower() == "auto":
            cli_slug = (getattr(self, "_aidiy_provider_slug", "") or "").lower()
            for cli_def in _AIDIY_CLI_PROVIDERS:
                if cli_def["slug"] == cli_slug:
                    model_short = cli_def["name"]
                    break
    ...
    @staticmethod
    def _parse_aidiy_model_args(raw_args: str) -> Tuple[str, Optional[str]]:
    ...
    def _current_aidiy_provider_slug(self) -> str:
        current = (getattr(self, "_aidiy_provider_slug", None) or "").strip().lower()
        if current:
            return current
        provider = (self.provider or self.requested_provider or "").strip().lower()
    ...
    def _get_aidiy_provider_entry(self, provider: str, include_models: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if provider == "ollama":
            api_key = str(cfg.get("ollama_key_id", "") or "")
            host = str(cfg.get("ollama_host", _DEFAULT_OLLAMA_HOST) or _DEFAULT_OLLAMA_HOST).rstrip("/")
            cloud = _is_valid_key(api_key)
            entry = {
                "slug": "ollama",
                "name": "Ollama Cloud" if cloud else "Ollama Local",
                "runtime_provider": "custom",
                "base_url": _OLLAMA_CLOUD_BASE_URL if cloud else f"{host}/v1",
                "api_key": api_key.strip() if cloud else "no-key-required",
                "api_mode": "chat_completions",
                "default_model": _strip_cloud_suffix(self._aidiy_provider_default_model("ollama")) if cloud else self._aidiy_provider_default_model("ollama"),
            }
        elif provider == "openai":
            api_key = str(cfg.get("openai_key_id", "") or "")
            if not _is_valid_key(api_key):
                return None
            entry = {
                "slug": "openai",
                "name": "OpenAI",
                # Upstream Hermes aliases bare "openai" to OpenRouter.  Use a
                # direct custom runtime so AiDiy's OpenAI key/base URL are used.
                "runtime_provider": "custom",
                "base_url": _OPENAI_BASE_URL,
                "api_key": api_key.strip(),
                "api_mode": "chat_completions",
                "default_model": self._aidiy_provider_default_model("openai"),
            }
        elif provider == "openrt":
            api_key = str(cfg.get("openrt_key_id", "") or "")
            if not _is_valid_key(api_key):
                return None
            entry = {
                "slug": "openrt",
        ...
        if include_models:
            entry["models"] = self._fetch_aidiy_provider_model_labels(entry, limit=50)
            if not entry["models"]:
                default_model = entry["default_model"]
                entry["models"] = [(f"yyyy/mm/dd - {default_model}", default_model)]
            entry["total_models"] = len(entry["models"])
        return entry
    ...
    def _dispatch_aidiy_cli_subprocess(self, message, images: list = None) -> Optional[str]:
        ...
        # Read stdout/stderr concurrently with readline() so output streams
        # in real time. ``for line in proc.stdout`` would use the iterator's
        # 8 KiB read-ahead buffer and only emit once enough bytes accumulate,
        # which made the live progress invisible for long CLI turns.
        import threading as _threading

        output_lines: list[str] = []
        stderr_lines: list[str] = []
        # Idle timeout: abort if no byte arrives on either stream for this many
        # seconds. Resets on every received line, so a long-running turn that
        # keeps emitting progress will not trip it. 5 minutes per user request.
        ...
        def _stderr_reader():
            try:
                ...
                logging.warning("CLI stderr read error: %s", exc)

        def _idle_monitor():
            while proc.poll() is None:
                time.sleep(1)
                if time.time() - last_output_time[0] > idle_timeout_sec:
                    timed_out[0] = True
                    try:
                        proc.kill()
                    except Exception as exc:
                        logging.warning("CLI idle-timeout kill failed: %s", exc)
                    return

        t_out = _threading.Thread(target=_stdout_reader, daemon=True)
        t_err = _threading.Thread(target=_stderr_reader, daemon=True)
        t_mon = _threading.Thread(target=_idle_monitor, daemon=True)
        t_out.start()
        t_err.start()
        t_mon.start()
        ...
        if timed_out[0]:
            msg = f"  {_DIM}[idle-timeout] no output for {idle_timeout_sec}s — aborted{_RST}"
            _cprint(msg)
        ...
        # Render the collected stdout inside the standard AiDiy,Hermes response
        # box so CLI output appears in the same shape as agent/chat responses.
    ...
    def _apply_aidiy_provider_model(self, provider_entry: Dict[str, Any], model_name: str) -> None:
        ...
        old_provider = (
            getattr(self, "_aidiy_provider_slug", None)
            or self._current_aidiy_provider_slug()
            or self.provider
        )
        old_model = self.model
        runtime_provider = provider_entry.get("runtime_provider") or self._aidiy_runtime_provider(provider_entry.get("slug", ""))
        self._aidiy_provider_slug = provider_entry.get("slug")
        ...
        _cprint(f"  Model switched: {old_provider}/{old_model} -> {provider_entry.get('slug')}/{self.model}")
        _cprint(f"    Provider: {provider_entry.get('name', provider_entry.get('slug', runtime_provider))}")
    ...
    def _open_aidiy_model_picker(self) -> None:
        providers = self._list_aidiy_provider_entries(include_models=True)
        if not providers:
        ...
        self._model_picker_state = {
            ...
            "providers": providers,
            "selected": selected,
            "current_provider": self._current_aidiy_provider_slug() or self.provider,
            "current_model": self.model,
        }
        self._invalidate(min_interval=0.0)

    def _handle_aidiy_model_command(self, cmd_original: str) -> bool:
        parts = cmd_original.split(None, 1)
        raw_args = parts[1].strip() if len(parts) > 1 else ""
        model_input, explicit_provider = self._parse_aidiy_model_args(raw_args)

        if not model_input and not explicit_provider:
            if self._app:
                self._open_aidiy_model_picker()
            else:
                self._show_aidiy_model_picker_fallback()
            return True

        if explicit_provider and explicit_provider not in self._aidiy_provider_slugs():
            return False
        ...
        if explicit_provider and not model_input:
            ...
            if not models:
                ...
                return True
            self._apply_aidiy_provider_model(provider_entry, models[0][1])
            return True
        ...
        if ":" in model_name and not provider_name:
            ...
            if prefix.lower() in self._aidiy_provider_slugs():
                provider_name = prefix.lower()
                model_name = rest.strip()

        provider_name = provider_name or self._current_aidiy_provider_slug() or self.provider or self.requested_provider
        provider_entry = self._get_aidiy_provider_entry(provider_name, include_models=False)
        if not provider_entry:
            return False
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if state.get("source") == "aidiy":
            ...
            if selected < len(model_list):
                ...
                self._close_model_picker()
                self._apply_aidiy_provider_model(provider_data, model_id)
                return
            self._close_model_picker()
            return
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        if self._handle_aidiy_model_command(cmd_original):
            return
...
def main(
    ...
    if query or image:
        ...
        if quiet:
            ...
            # Only print the final response to stdout. Operational output
            # (thinking, tool steps, warnings, session metadata) goes to stderr
            # so AiDiy can treat stdout as the official answer channel.
            cli.tool_progress_mode = "off"
            result = None
            response = ""
            exit_code = 1
            from contextlib import redirect_stdout
            with redirect_stdout(sys.stderr), _route_cprint_to_stderr():
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        cli.agent.thinking_callback = None
                        cli.agent.tool_progress_callback = None

                        def _stderr_progress_print(*a, **kw):
                            print(*a, **{**kw, "file": kw.get("file", sys.stderr)})

                        _stderr_progress_print._static_spinner = True
                        cli.agent._print_fn = _stderr_progress_print
                        # Suppress streaming display callbacks so stdout stays
                        # machine-readable (no styled "Hermes" box, no tool-gen
                        # response text).  Thinking/tool progress uses stderr
                        # via _print_fn above. The response is printed once
                        # below to stdout.
...
def _build_arg_parser():
    ...
    parser.add_argument("--list-toolsets", action="store_true", default=False)
...
def _flatten_skills(skills: list[str] | None) -> str | None:
    if not skills:
        return None
    return ",".join(skills)
...
def _load_aidiy_hermes_defaults(requested_model: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        "api_key": str(cfg.get("ollama_key_id") or "no-key-required").strip() or "no-key-required",
        "model": _resolve_aidiy_ollama_model(host, model),
    }
...
def _resolve_aidiy_ollama_model(host: str, configured_model: str) -> str:
    ...
    try:
        ...
        if model in names:
            return model
        cloud_dash = re.sub(r":cloud$", "-cloud", model)
        if cloud_dash in names:
            return cloud_dash
        cloud_colon = re.sub(r"-cloud$", ":cloud", model)
        if cloud_colon in names:
            return cloud_colon
...
def cli_entry(argv: list[str] | None = None) -> int:
    """Console-script entry point for aidiy_hermes."""
    ...
    if args.version:
        try:
            ...
            print(f"aidiy_hermes v{__version__} ({__release_date__})")
        except Exception:
            print("aidiy_hermes")
        return 0
    ...
    try:
        main(
            ...
            skills=_flatten_skills(args.skills),
            model=model,
            provider=provider,
            api_key=api_key,
            base_url=base_url,
...
def text_main(argv: list[str] | None = None) -> int:
    """Console-script entry point for `text-hermes PROMPT`."""
    argv = list(sys.argv[1:] if argv is None else argv)
    if not argv or argv[0] in {"-h", "--help"}:
        print("usage: text-hermes PROMPT")
        return 0 if argv else 2
    return cli_entry(["-Q", "-z", " ".join(argv)])
