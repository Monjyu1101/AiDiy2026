...
# instead of the default console readers. The CLI registers these so prompts
# route through prompt_toolkit's event loop.
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    directly from the Windows console with echo disabled.
    """
    import sys
    ...
    def read_password_thread():
        """Read password from the Windows console with echo disabled."""
        try:
            import msvcrt
            chars = []
            while True:
...
_WINDOWS_DESCRIPTION = (
    "On Windows hosts Hermes prefers Git Bash when present and otherwise falls "
    "back to PowerShell; use portable commands (`pwd`, `ls`, `test -e path`, "
    "`git`, `python`, `npm`) and prefer file tools for file inspection/editing. "
    "On Windows, use timeout >= 60 for shell commands unless the command is "
    "trivial; Hermes enforces a minimum safeguard for shorter values."
)

TERMINAL_TOOL_DESCRIPTION = f"""Execute shell commands in the configured local shell. Filesystem usually persists between calls. {_WINDOWS_DESCRIPTION}
...
def terminal_tool(
    ...
    try:
        ...
        if timeout is not None and timeout < 30:
            logger.info(
                "[winnt] terminal timeout raised from %ss to 30s for command startup tolerance",
                timeout,
            )
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            try:
                from tools.environments.local import _find_windows_shell
                shell_kind, shell_path = _find_windows_shell()
                logger.info("[winnt] local shell selected: %s (%s)", shell_kind, shell_path)
                return True
            except Exception as exc:
                logger.error("[winnt] local shell validation failed: %s", exc)
                return False
