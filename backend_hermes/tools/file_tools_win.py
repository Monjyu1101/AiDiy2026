...
from tools.file_operations_linux import ShellFileOperations
from tools.file_operations_win import (
...
# Windows has no Unix device files to block here.
...
def _is_blocked_device(filepath: str) -> bool:
    """Return True if the path is a blocked Windows device path."""
...
def _get_file_ops(task_id: str = "default"):
    """Get or create file operations for a terminal environment.
    ...
    # Windows-native local paths use direct pathlib/subprocess operations.
    # Remote/container backends expose POSIX shells, so keep shell operations there.
    if getattr(terminal_env, "_winnt_native_local", False):
        file_ops = WindowsFileOperations(terminal_env)
    else:
