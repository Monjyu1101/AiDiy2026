...
class KawaiiSpinner:
    ...
    def _static_wait_message(self) -> str:
        """Compact non-animated wait label for one-shot stderr logs."""
        text = (self.message or "").strip()
        if not text:
            return "処理待機"

        # Thinking spinners are usually "<face> formulating..."; collapse them
        # to a stable Japanese label instead of leaking random verb choices.
        lower = text.lower()
        if any(re.search(rf"\b{re.escape(verb)}\b", lower) for verb in self.THINKING_VERBS):
            return "AI応答待機"

        # Tool spinners are usually "<face> <emoji> <preview>". Remove the
        # decorative prefix but keep the actionable command/tool preview.
        for face in self.KAWAII_WAITING + self.KAWAII_THINKING:
            if text.startswith(face):
                text = text[len(face):].strip()
                break
        text = re.sub(r"^[^\w./\\~:\-$@]+", "", text).strip()
        text = re.sub(r"\s+", " ", text)
        return f"{text or '処理'}待機"

    def _animate(self):
        # One-shot automation can route progress to stderr through print_fn.
        # In that mode CR-based animation becomes one log line per frame, so
        # emit only a single wait line and let stop() emit the completion.
        if getattr(self._print_fn, "_static_spinner", False):
            self._write(f"  [wait] {self._static_wait_message()}", flush=True)
            while self.running:
                time.sleep(0.5)
            return
