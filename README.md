...
2. **フロントエンド環境準備 (npm install)**
   - `frontend_web/` の Node.js パッケージをインストール
...
- **ポート変更時の連動修正**: フロントエンドのポートを変える場合、`frontend_web/vite.config.ts` に加えて `backend_server/core_main.py` と `backend_server/apps_main.py` の CORS 許可リスト、`_start.py` のポート設定も更新が必要です。
...
**`frontend_web/.vscode/launch.json`:**
...
4. **[./frontend_web/AGENTS.md](./frontend_web/AGENTS.md)** - フロントエンド実装詳細
...
本プロジェクトはソースコード公開型ですが、ライセンスは **非商用利用限定** です。

- 非商用利用（学習、検証、社内評価など）は許可
- 商用利用（業務利用、有償サービス、受託開発、SaaS/ASP提供など）は禁止
- 商用利用する場合は、**事前に全著作権者（諸作権者）の書面承諾が必須**
- ライセンス条項は、著作権者により**予告なく変更される場合があります**
