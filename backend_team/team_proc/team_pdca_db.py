...
def 次のPDCA区分(プロジェクト: str) -> str:
    ...
    - 未終了のレコードが1件でもあれば空文字（前の段がまだ終わっていない）
    - 最終段の全レコードの状況が「済」または「エラー」でなければ空文字
    - レコードが無ければ 'S'（相談から始める）
    ...
    if 未終了一覧(プロジェクト):
        return ""
    if any(str(row.get("状況", "")) not in ("済", "エラー") for row in 最終段):
        return ""
            ...
            UPDATE "{改善テーブル}"
             ...
             WHERE 改善ID IN (
                   SELECT p.改善ID
                    ...
                    WHERE p.終了日時 = ''
                      AND w.状態 IN ('済', 'エラー', '中止'){条件}
             )
            """,
            [now, now, *([プロジェクト] if プロジェクト else [])],
        )
        conn.commit()
