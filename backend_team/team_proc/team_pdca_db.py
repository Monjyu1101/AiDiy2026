...
Aチーム目標の `目標ループ` がオンのプロジェクトについて、空き時間に PDCA を1段ずつ
...
def 作業一覧(プロジェクト: str = "", 件数: int = 一覧最大件数) -> list[dict]:
    ...
    try:
        ...
        rows = conn.execute(
            f"""
            SELECT 作業ID, プロジェクト, ループ, 依頼ID, チーム目標, 要員ID, PDCA区分, 状況,
...
def 最終段一覧(プロジェクト: str) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            f"""
            SELECT 作業ID, プロジェクト, ループ, 依頼ID, チーム目標, 要員ID, PDCA区分,
...
def 目標ループ実行中(プロジェクト: str, パターン: str, 最大ループ回数: int) -> bool:
    """目標ループがまだ動いているかを返す（掲示板のネオン点灯の切り替え用）。

    最大ループ回数まで回り、その最終ループの最終段（SPDCA=A / PlanDo=D）が
    済またはエラーで決着していれば、次のループは投入されない
    （`sub_*_plan.py` の次ループ番号が0を返す）ため停止とみなす。

    判定に件数は使わない。途中のループがエラーで終わっていても後続ループが済に
    なることがあり、件数の突き合わせでは今どこまで進んだかを取り違えるため、
    最終段のレコードがある最大ループの状況だけを見る。
    99（無制限）は止まらないので常に実行中を返す。
    """
    初期化()
    if 最大ループ回数 == 99:
        return True
    ...
    if ループ数 < 最大ループ回数:
        # まだ上限に達していないので、次のループが投入される
        return True
    if not 最新 or int(最新["ループ"] or 0) < ループ数:
        # 最終段が最新ループまで来ていない（Pだけ投入された等）
        return True
    return str(最新["状況"] or "") not in 段終了状況
...
def 作業登録(データ: dict) -> dict:
    ...
    try:
        ...
        conn.execute(
            f"""
            INSERT INTO "{作業テーブル}" (
                作業ID, プロジェクト, ループ, 依頼ID, チーム目標, 要員ID, PDCA区分, 状況,
                開始日時, 終了日時, 応答内容,
                登録日時, 登録利用者ID, 登録利用者名, 登録端末ID,
                更新日時, 更新利用者ID, 更新利用者名, 更新端末ID
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, '', '', ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    try:
        ...
        if 既存:
            ...
            conn.execute(
                f"""
                INSERT INTO "{作業テーブル}" (
                    作業ID, プロジェクト, ループ, 依頼ID, チーム目標, 要員ID,
...
def 取り残し終了(プロジェクト: str = "") -> int:
    ...
    2. Aチーム依頼が「完了」のまま滞留しているもの
       ...
       経験レコードが無いものだけを拾うため）ので、ここで回収しないと目標ループが
...
def 作業クリア(プロジェクト: str) -> int:
    """そのプロジェクトの作業記録をすべて削除する（目標ループのオン更新時）。"""
    初期化()
    conn = 接続取得()
    try:
        cursor = conn.execute(
