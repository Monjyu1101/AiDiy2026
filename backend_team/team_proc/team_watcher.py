...
def _作業実行開始(目標: dict, 区分: str, logger: logging.Logger) -> subprocess.Popen | None:
    ...
    try:
        ...
        logger.info(
            f"目標ループ({区分})の投入を開始しました: プロジェクト={プロジェクト} PID={proc.pid}"
        )
        return proc
    except Exception:
        logger.exception(f"目標ループ({区分})の起動に失敗しました: プロジェクト={プロジェクト}")
        return None
...
def _目標ループ確認(logger: logging.Logger) -> None:
    """目標ループがオンの目標について、空き時間なら次の PDCA 段を投入する（1分ごと）。
    ...
    try:
        ...
        logger.exception("目標ループの確認でエラーが発生しました")
...
async def 状態監視ループ(logger: logging.Logger) -> None:
    """状態監視タイマー：無進捗タイムアウト・経験生成・目標ループを確認する。
