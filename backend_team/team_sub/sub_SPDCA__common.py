...
# プロンプト生成関数の形: (要員ID, プロジェクト, チーム目標, 区分ごとのまとめ内容) -> 要求内容
...
def 投入失敗を記録(区分: str, プロジェクト: str, チーム目標: str, 理由: str, logger) -> None:
    ...
    繰り返して目標ループが進まなくなる（team_watcher._目標ループ確認 は未終了レコードの
    有無で次の段を決めるため）。
    """
    if not プロジェクト:
        # プロジェクトが取れない＝入力JSONすら読めていない場合は記録先を決められない
        logger.warning(f"目標ループ({区分})はプロジェクト不明のため失敗レコードを残せません")
        return
    try:
        実行不能を記録(
            区分, プロジェクト, チーム目標,
        ...
        logger.exception(f"目標ループ({区分})の失敗レコード作成にも失敗しました")
...
def 段を実行(
    ...
    残して 0 を返す（未投入のままだと毎分この段の投入が再試行され、目標ループが止まるため）。
    ...
    try:
        ...
        if 要員継続 and 前段要員ID:
            要員ID = 前段要員ID
            logger.info(f"目標ループ({区分})は前段({前段区分})の担当要員を引き継ぎます: 要員ID={要員ID}")
        else:
        ...
        if 欠落:
            logger.warning(
                f"目標ループ({区分})は同ループの参照内容を取得できませんでした: 区分={','.join(欠落)}"
            )
        logger.info(
        ...
        logger.exception(f"目標ループ({区分})の投入処理に失敗しました")
