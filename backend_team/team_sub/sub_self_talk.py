...
これらを踏まえ、あなた自身の考えとして「今やるべきこと」を、自然な口調で1〜3文にまとめて発言してください。
「何を対象に」「どのような行動を取り」「何を確認するか」が分かる、すぐ次の行動に移せる具体的な提案にしてください。
プロジェクト内の機能名、画面名、API、ファイル・フォルダ、または調査項目など、目標に関係する具体的な対象を少なくとも1つ挙げてください。
抽象的な方針や「検討する」「改善する」だけで終わらせず、不明な点がある場合は最初に確認する対象と確認方法を具体化してください。
長い説明や前置きは不要です。他の要員の発言と重複しにくい観点があれば、それを優先してください。確認できない固有名詞や数値は作らないでください。
"""
...
def main() -> int:
    ...
    try:
        ...
        依頼内容 = 依頼内容を作る(チーム目標, 他者意見一覧, 自身の前回発言)
