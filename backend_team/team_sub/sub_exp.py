...
def JSON形式サンプル(経験ID: str, 作業ID: str) -> str:
  ...
  "経験値": 35,
  "分類": "実装",
  "経験内容": "何をどう進めて、どこまで到達したかの具体的なまとめ",
...
- 経験内容は、対象（ファイル名・機能名・画面名・API名など）と、行った変更・確認の内容、
...
まとめの内容（タイトル・経験値・分類・経験内容・学び）は変更しないでください。
...
def JSON検証(データ: dict, 作業ID: str) -> dict:
    ...
    for キー in ("経験ID", "作業ID", "タイトル", "経験値", "分類", "経験内容", "学び"):
        if キー not in データ:
    ...
    経験内容 = str(データ["経験内容"]).strip()
    if not タイトル:
        raise ValueError("タイトルが空です")
    if not 経験内容:
        raise ValueError("経験内容が空です")
    try:
