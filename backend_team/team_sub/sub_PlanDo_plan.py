...
3. Aチーム作業（状態=準備中）→ Aチーム改善（開始レコード）→
   aidiy_task_agents への投入（Aタスク要求）の順で作る
4. 投入に成功した作業は 準備完了 にする。失敗した場合は対応する
   Aチーム改善レコードも終了させて次のサイクルを止めない
"""
...
def main() -> int:
    ...
    try:
        ...
        if not ループ:
            logger.info(
                f"改善ループ(P)は最大回数到達のため投入しません: プロジェクト={プロジェクト} "
