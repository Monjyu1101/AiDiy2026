...
処理の流れ:
1. 入力 JSON（プロジェクト / チーム作業 / PDCA区分 / 最大ループ回数）を読み込む
...
def 計画を投入(
    ...
    return sub_SPDCA__common.段を投入(
        ...
        チーム作業,
        ループ,
        プロンプト生成_計画(要員ID, プロジェクト, チーム作業, 前サイクル進捗),
        logger,
    )
...
def main() -> int:
    ...
    try:
        ...
        if not ループ:
            logger.info(
                f"作業ループ(P)は最大回数到達のため投入しません: プロジェクト={プロジェクト} "
        ...
        選択用要求内容 = f"チーム作業「{チーム作業}」に向けた次の実行計画を立案する（プロジェクト: {プロジェクト}）。"
        if 前サイクル進捗:
            選択用要求内容 += f"\n\n前サイクルのD（実行）の実施内容:\n{前サイクル進捗}"
        ...
        成功 = False
        try:
            成功 = 計画を投入(要員ID, プロジェクト, チーム作業, 区分, ループ, logger, 前サイクル進捗)
        except Exception:
