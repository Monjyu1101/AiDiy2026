...
その 2 つを呼び出し側（`sub_pdca_do.py` / `sub_pdca_check.py` / `sub_pdca_action.py`）が渡す。

S と P はメイン処理の流れが違うため、それぞれの実装（`sub_pdca_soudan.py` /
`sub_pdca_plan.py`）に残し、上の共通部品だけを使っている。
...
def 段を実行(区分: str, 前段区分: str, プロンプト生成: プロンプト生成関数, ログ名: str) -> int:
