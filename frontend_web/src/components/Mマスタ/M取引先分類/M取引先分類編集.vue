...
const requiredFields = computed(() => ['取引先分類ID', '取引先分類名']);
...
const applyDataToForm = (data) => {
  ...
  form.取引先分類名 = data?.取引先分類名 || '';
  form.取引先分類備考 = data?.取引先分類備考 || '';
...
const validateForm = () => {
  ...
  if (firstErrorField) {
    const fieldMap = {
      ...
      '取引先分類名': 'form-partner-category-name'
    };
...
const loadDetail = async (取引先分類ID) => {
  message.value = '';
  try {
    const res = await apiClient.post('/apps/M取引先分類/取得', { 取引先分類ID: String(取引先分類ID) });
    if (res.data.status === 'OK' && res.data.data) {
      detailData.value = res.data.data;
      applyDataToForm(res.data.data);
    } else {
      showMessage(res.data.message || '取引先分類情報の取得に失敗しました。', 'error');
    }
  } catch (e) {
    showMessage('取引先分類情報の取得でエラーが発生しました。', 'error');
  }
};
...
const applyQueryParams = async (query) => {
  ...
  if (query.モード === '表示' && query.取引先分類ID) {
    mode.value = 'view';
    await loadDetail(query.取引先分類ID);
    return;
  }
...
const backToList = () => {
  router.push({ path: '/Mマスタ/M取引先分類/一覧', query: buildListQuery() });
};
...
const saveData = async () => {
  ...
  try {
    ...
    if (isCreateMode.value) {
      res = await apiClient.post('/apps/M取引先分類/登録', {
        取引先分類ID: form.取引先分類ID,
        取引先分類名: form.取引先分類名,
        取引先分類備考: form.取引先分類備考,
        有効: form.有効
      });
    } else {
      res = await apiClient.post('/apps/M取引先分類/変更', {
        取引先分類ID: form.取引先分類ID,
        取引先分類名: form.取引先分類名,
        取引先分類備考: form.取引先分類備考,
        有効: form.有効
...
const deleteData = async () => {
  if (!form.取引先分類ID) return;

  const confirmed = await qConfirm(`M取引先分類「${form.取引先分類ID}」を削除しますか？この操作は取り消せません。`);
  if (!confirmed) return;

  try {
    const res = await apiClient.post('/apps/M取引先分類/削除', { 取引先分類ID: form.取引先分類ID });
    if (res.data.status === 'OK') {
      handleSuccess(res.data.message);
    } else {
...
<template>
  <div class="page-container">
    <h2 class="page-title">
      <span class="title-text">【 M取引先分類 】</span>
          ...
          <div class="detail-panel">
            <template v-if="activeTab === 'content'">
              <div class="detail-row row-id">
                <div class="detail-label">取引先分類ID<span class="required-mark">*</span></div>
                <div class="detail-value">
                  <div class="value-column">
                    <div class="input-wrap">
                      <input
                        id="form-partner-category-id"
                        type="text"
                        v-model="form.取引先分類ID"
                        :readonly="!isCreateMode || isViewMode"
                        class="detail-input id-input"
                        :class="{ 'input-error': errors.取引先分類ID }"
                        @blur="handleBlur('取引先分類ID')"
                        @input="handleInput('取引先分類ID')"
                      />
              ...
              <div class="detail-row row-name">
                <div class="detail-label">取引先分類名<span class="required-mark">*</span></div>
                <div class="detail-value">
                  <div class="value-column">
                    <div class="input-wrap">
                      <input
                        id="form-partner-category-name"
                        type="text"
                        v-model="form.取引先分類名"
                        class="detail-input name-input"
                        :class="{ 'input-error': errors.取引先分類名 }"
                        :readonly="isViewMode"
                        @blur="handleBlur('取引先分類名')"
                        @input="handleInput('取引先分類名')"
                      />
                      <span v-if="errors.取引先分類名" class="input-alert">!</span>
                    </div>
                    <div v-if="errors.取引先分類名 && errors.取引先分類名 !== 'ERROR'" class="field-error">{{ errors.取引先分類名 }}</div>
                  </div>
                </div>
              </div>

              <div class="detail-row row-remarks">
                <div class="detail-label">取引先分類備考</div>
                <div class="detail-value">
                  <div class="input-wrap">
                    <textarea
                      id="form-partner-category-remarks"
                      v-model="form.取引先分類備考"
