...
import type { Column, V取引先分類 } from '../../../../types';
...
const 取引先分類一覧 = ref<V取引先分類[]>([]);
const serverTotal = ref(0);
const pageSize = ref(100);
const currentPage = ref(1);
const sortKey = ref('取引先分類ID');
...
const rowKey = '取引先分類ID';
const columns = computed<Column[]>(() => {
  const baseColumns: Column[] = [
    { key: '取引先分類ID', label: '取引先分類ID', width: '120px', sortable: true, align: 'center' },
    { key: '取引先分類名', label: '取引先分類名', width: '200px', sortable: true },
    { key: '取引先分類備考', label: '取引先分類備考', width: '220px', sortable: true },
...
const filteredRows = computed(() => {
  return 取引先分類一覧.value.filter((row) => {
...
const openDetail = (row) => {
  const query: Record<string, string> = { モード: '編集', 取引先分類ID: row.取引先分類ID };
  if (props.戻URL) {
    query.戻URL = props.戻URL;
  }
  router.push({ path: '/Mマスタ/M取引先分類/編集', query });
};

const loadData = async () => {
  message.value = '';
  try {
    const res = await apiClient.post('/apps/V取引先分類/一覧', {
      件数制限: props.件数制限,
      無効も表示: props.無効も表示
    });
    if (res.data.status === 'OK') {
      ...
      setMessage(res.data.message || 'M取引先分類一覧の取得に失敗しました。', 'error');
    }
  } catch (e) {
    setMessage('M取引先分類一覧の取得でエラーが発生しました。', 'error');
  }
};
...
<template>
  ...
  >
    ...
    <template #cell="{ row, column, value }">
      <template v-if="column.key === '取引先分類ID' || column.key === '取引先分類名'">
