...
const requiredFields = computed(() => ['生産工程ID', '生産工程名']);
...
const applyDataToForm = (data) => {
  ...
  form.生産工程名 = data?.生産工程名 || '';
  form.生産工程備考 = data?.生産工程備考 || '';
...
const loadDetail = async (processId) => {
  message.value = '';
  try {
    const res = await apiClient.post('/apps/M生産工程/取得', { 生産工程ID: String(processId) });
    if (res.data.status === 'OK' && res.data.data) {
      detailData.value = res.data.data;
      applyDataToForm(res.data.data);
    } else {
      showMessage(res.data.message || '生産工程情報の取得に失敗しました。', 'error');
    }
  } catch (e) {
    showMessage('生産工程情報の取得でエラーが発生しました。', 'error');
  }
};
...
const applyQueryParams = async (query) => {
  ...
  if (query.モード === '表示' && query.生産工程ID) {
    mode.value = 'view';
    await loadDetail(query.生産工程ID);
    return;
  }
...
const backToList = () => {
  router.push({ path: '/Mマスタ/M生産工程/一覧', query: buildListQuery() });
};
...
const saveData = async () => {
  ...
  try {
    ...
    if (isCreateMode.value) {
      res = await apiClient.post('/apps/M生産工程/登録', {
        生産工程ID: form.生産工程ID,
        生産工程名: form.生産工程名,
        生産工程備考: form.生産工程備考,
        有効: form.有効
      ...
      res = await apiClient.post('/apps/M生産工程/変更', {
        生産工程ID: form.生産工程ID,
        生産工程名: form.生産工程名,
        生産工程備考: form.生産工程備考,
        有効: form.有効
...
const deleteData = async () => {
  if (!form.生産工程ID) return;

  const confirmed = await qConfirm(`M生産工程「${form.生産工程ID}」を削除しますか？この操作は取り消せません。`);
  if (!confirmed) return;

  try {
    const res = await apiClient.post('/apps/M生産工程/削除', { 生産工程ID: form.生産工程ID });
    if (res.data.status === 'OK') {
      handleSuccess(res.data.message);
    } else {
          ...
          <div class="detail-panel">
            <template v-if="activeTab === 'content'">
              <div class="detail-row row-id">
                <div class="detail-label">生産工程ID<span class="required-mark">*</span></div>
              ...
              <div class="detail-row row-name">
                <div class="detail-label">生産工程名<span class="required-mark">*</span></div>
                <div class="detail-value">
                  <div class="value-column">
                    <div class="input-wrap">
                      <input
                        ...
                        :class="{ 'input-error': errors.生産工程名 }"
                        :readonly="isViewMode"
                        @blur="handleBlur('生産工程名')"
                        @input="handleInput('生産工程名')"
                      />
                      <span v-if="errors.生産工程名" class="input-alert">!</span>
                    </div>
                    <div v-if="errors.生産工程名 && errors.生産工程名 !== 'ERROR'" class="field-error">{{ errors.生産工程名 }}</div>
                  </div>
                </div>
              </div>

              <div class="detail-row row-remarks">
                <div class="detail-label">生産工程備考</div>
                <div class="detail-value">
                  <div class="input-wrap">
                    <textarea
