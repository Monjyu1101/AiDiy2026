...
import type { Column, V生産工程 } from '../../../../types';
...
const 生産工程一覧 = ref<V生産工程[]>([]);
const serverTotal = ref(0);
const pageSize = ref(100);
const currentPage = ref(1);
const sortKey = ref('生産工程ID');
...
const columns: Column[] = [
  { key: '生産工程ID', label: '生産工程ID', width: '120px', sortable: true, align: 'center' },
  { key: '生産工程名', label: '生産工程名', width: '200px', sortable: true },
  { key: '生産工程備考', label: '生産工程備考', width: '220px', sortable: true },
...
const filteredRows = computed(() => {
  return 生産工程一覧.value.filter((row) => {
...
const openDetail = (row) => {
  const query: Record<string, string> = { モード: '編集', 生産工程ID: row.生産工程ID };
  if (props.戻URL) {
...
const loadData = async () => {
  message.value = '';
  try {
    const res = await apiClient.post('/apps/V生産工程/一覧', {
      件数制限: props.件数制限,
      無効も表示: props.無効も表示
    });
    if (res.data.status === 'OK') {
      ...
      setMessage(res.data.message || 'M生産工程一覧の取得に失敗しました。', 'error');
    }
  } catch (e) {
    setMessage('M生産工程一覧の取得でエラーが発生しました。', 'error');
  }
};
...
<template>
  ...
  >
    ...
    <template #cell="{ row, column, value }">
      <template v-if="column.key === '生産工程ID' || column.key === '生産工程名'">
