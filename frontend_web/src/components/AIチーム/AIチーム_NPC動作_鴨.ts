// NPC鴨の造形と動作。親鴨（鴨大）1羽と子鴨（鴨小）5羽が水場のまわりで暮らす。
//
// 親鴨は水場の岸を散歩し、ときどき水へ入って泳ぐ。陸か水かは「水場の中心からの距離」だけで
// 判定するので、目的地を水の中に選べば岸を越えてそのまま入水し、陸に選べば上がる。
//
// 子鴨の隊列は、鴨ごとに違う「これ以上親へ近づかない距離」だけで作る。
...
/** 親子で共通の見た目と速さ */
type 鴨共通設定 = {
  /** 体格の倍率（鴨大=1、鴨小はこれを小さくする） */
  体格: number;
  /** 頭の大きさを体格から更に補正する。子鴨は頭を大きめにすると雛らしく見える */
  頭径倍率: number;
  体色: number;
  頭色: number;
  くちばし色: number;
...
export type 鴨大設定 = 鴨共通設定 & {
  /** 岸からどれだけ外側まで散歩するか */
  散歩幅: number;
  /** 各状態の滞在時間 [最小秒, 最大秒] */
  滞在時間: Record<鴨状態, [number, number]>;
};

export type 鴨小設定 = 鴨共通設定 & {
  /** ついていく親鴨 */
  親: THREE.Group | null;
...
const 共通既定: Omit<鴨共通設定, '体格' | '頭径倍率' | '体色' | '頭色' | '首輪色'> = {
  くちばし色: 0xe8a33d,
  水かき色: 0xe0913a,
  歩く速度: 0.62,
  泳ぐ速度: 0.5,
...
/** 鴨の体を組む。前方は -z（ほかの NPC と同じ向き付け） */
const 鴨体を組む = (
  設定: 鴨共通設定,
  ヘルパー: 造形ヘルパー,
): { group: THREE.Group; 部位: 鴨部位 } => {
  ...
  // 尾は尻から後ろへ出しつつ先を上へ跳ね上げる（鴨らしいシルエットになる）
...
const 鴨を生成 = (
  ...
  const 更新 = (引数: NPC更新引数) => {
    ...
    const { 目的地, 速度倍率 = 1 } = 行動(引数, { 水中, 位置: group.position });
...
export const 鴨大定義: NPC定義<鴨大設定> = {
  ...
  生成: (scene, ヘルパー, 配置, 設定): NPC個体 => {
    ...
    const 目的地を選ぶ = (状態: 鴨状態): THREE.Vector3 => {
    ...
    return 鴨を生成('鴨大', scene, ヘルパー, 配置, 設定, ({ delta }) => {
...
export const 鴨小定義: NPC定義<鴨小設定> = {
  種別: '鴨小',
  既定設定: {
    ...共通既定,
    // 体長 0.62 * 0.30 ≒ 0.19m。子鴨どうしの車間（既定 0.2m 刻み）より短くして、
  ...
  生成: (scene, ヘルパー, 配置, 設定): NPC個体 =>
    鴨を生成('鴨小', scene, ヘルパー, 配置, 設定, ({ delta }, { 位置 }) => {
      const 親 = 設定.親;
