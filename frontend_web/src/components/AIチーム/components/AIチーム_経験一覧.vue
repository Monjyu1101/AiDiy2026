...
const 更新確認 = async () => {
  try {
    ...
    if (maxUpdatedAt !== 経験最大更新日時) await 経験一覧読込();
  } catch {
...
// ダブルクリックで 要求内容 / 応答内容 / 経験内容 を共通ダイアログに表示する
const 内容を開く = (経験: チーム経験) => {
  ...
  const 経験内容 = [
    ...
    .filter((行) => 行 !== '')
    .join('\n');
  if (!要求.trim() && !応答.trim() && !経験内容.trim()) return;
...
<template>
  ...
  >
    ...
    <div class="table-frame">
      ...
      <div v-else class="panel-hint">行をダブルクリックで要求・応答・経験内容を表示</div>
    </div>
  </aside>
