...
const loadSchedules = async () => {
  if (!表示日付.value) return;
  try {
    const response = await apiClient.post('/apps/S生産_日表示/生産一覧', {
...
const handleDropUpdate = async ({ scheduleId, processId, timeSlot }) => {
  ...
  try {
    ...
    if (response.data.status === 'OK') {
      ...
      showMessage(response.data.message || '生産予定の更新に失敗しました。', 'error');
    }
  } catch (error) {
    showMessage('生産予定の更新でエラーが発生しました。', 'error');
  }
  await loadSchedules();
};

const openEditForm = (scheduleId = null, processId = null, timeSlot = null) => {
  saveListSession();
  ...
  if (scheduleId) {
    const queryString = `生産伝票ID=${encodeURIComponent(scheduleId)}&${returnQuery}`;
    router.push(`/Tトラン/T生産/編集?${queryString}`);
  } else if (processId && timeSlot) {
    ...
    const queryString = `モード=${encodeURIComponent('新規')}&生産工程ID=${encodeURIComponent(String(processId))}&生産開始日時=${encodeURIComponent(startDateTime)}&生産終了日時=${encodeURIComponent(endDateTime)}&${returnQuery}`;
    router.push(`/Tトラン/T生産/編集?${queryString}`);
  }
};
...
<template>
  <div class="page-container">
    <h2 class="page-title">
      <span class="title-text">【 S生産_日表示 】</span>
