...
const 明細一覧 = ref<棚卸明細Form[]>([]);

const isCreateMode = computed(() => mode.value === 'create');
const isEditMode = computed(() => mode.value === 'edit');
const isViewMode = computed(() => mode.value === 'view');
const requiredFields = computed(() => ['棚卸日']);
...
const createEmptyDetail = (明細SEQ = 1, 商品ID = ''): 棚卸明細Form => ({
  明細SEQ,
  商品ID,
  実棚数量: '',
  明細備考: ''
...
const handleQuantityFocus = async (row: 棚卸明細Form, event: FocusEvent) => {
  数量編集中[row.明細SEQ] = true;
  row.実棚数量 = normalizeNumericInput(row.実棚数量);
  await nextTick();
  const target = event.target as HTMLInputElement | null;
  target?.select();
};

const handleQuantityInput = (row: 棚卸明細Form, event: Event) => {
  const target = event.target as HTMLInputElement | null;
  row.実棚数量 = normalizeNumericInput(target?.value ?? '');
};

const handleQuantityBlur = (row: 棚卸明細Form) => {
  row.実棚数量 = normalizeNumericInput(row.実棚数量);
...
const applyDataToForm = (data: any) => {
  ...
  明細一覧.value = Array.isArray(data?.明細一覧) && data.明細一覧.length
    ? data.明細一覧.map((item: any, index: number) => ({
      ...
      商品ID: item?.商品ID || '',
      実棚数量: item?.実棚数量 === null || item?.実棚数量 === undefined ? '' : String(item.実棚数量),
      明細備考: item?.明細備考 || ''
    }))
    : [];
...
const sanitizeDetails = () => {
  return 明細一覧.value
    .map((row, index) => ({
      ...
      実棚数量: String(row.実棚数量 || '').trim(),
      明細備考: String(row.明細備考 || '').trim()
    }))
    .filter((row) => row.商品ID || row.実棚数量 || row.明細備考);
};
...
const validateDetails = () => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!row.実棚数量) {
      detailError.value = `${rowNo}行目の実棚数量を入力してください。`;
      return null;
    }
    const 数量 = toNumber(row.実棚数量);
    if (!Number.isInteger(数量) || 数量 < 0) {
      detailError.value = `${rowNo}行目の実棚数量は0以上の整数を入力してください。`;
      return null;
    }
  }
...
const validateForm = () => {
  ...
  if (firstErrorField) {
    const fieldMap: Record<string, string> = { '棚卸日': 'form-date' };
...
const applyQueryParams = async (query: any) => {
  ...
  if (query.モード === '表示' && query.棚卸伝票ID) {
    mode.value = 'view';
    await loadDetail(String(query.棚卸伝票ID));
    return;
  }
...
const saveData = async () => {
  ...
  try {
    ...
    if (isCreateMode.value) {
      res = await apiClient.post('/apps/T商品棚卸/登録', payload);
    } else {
      res = await apiClient.post('/apps/T商品棚卸/変更', payload, {
...
const deleteData = async () => {
  ...
  const confirmed = await qConfirm(`T商品棚卸「${form.棚卸伝票ID}」の有効をオフにしますか？`);
  if (!confirmed) return;
            ...
            <template v-if="activeTab === 'content'">
              <div v-if="!isCreateMode" class="detail-row row-id">
                ...
                <div class="detail-value">
                  <input type="text" v-model="form.棚卸伝票ID" class="detail-input id-input center-input" readonly />
                </div>
              </div>
              ...
              <div class="detail-row row-remarks">
                ...
                <div class="detail-value">
                  <textarea v-model="form.棚卸備考" class="detail-textarea remarks-textarea" rows="2" :readonly="isViewMode"></textarea>
                </div>
              </div>

              <div class="composition-block">
                <div class="composition-header">
                  <div class="composition-title">棚卸明細</div>
                  ...
                  <table class="composition-table">
                    ...
                    <tbody>
                      ...
                      <tr v-for="(row, index) in 明細一覧" :key="`detail-${index}`">
                        ...
                        <td>
                          <input
                            :value="isQuantityEditing(row.明細SEQ) ? row.実棚数量 : formatNumericDisplay(row.実棚数量)"
                            type="text"
                            inputmode="numeric"
