...
- AIチーム画面は `components/AIチーム/AIチーム.vue` を軽量な親とし、メンバー・立体表示・作業一覧・経験一覧・改善一覧のコンポーネントへ責務を分ける。経験一覧は掲示板のプロジェクトだけを「何を／誰が／完了日時」で表示し、行ダブルクリックで `dialog/AIチーム_応答内容.vue`（要求内容・応答内容・まとめ内容の共通ダイアログ）を開く。改善一覧（`AIチーム_改善一覧.vue`、`/team/改善/*`）は掲示板の目標の `改善ループ` がオンのときだけ画面下・中央に出し、改善ループ（`S`相談 → `P`計画 → `D`実行 → `C`評価 → `A`改善）の実行状況を「何を（PDCA区分）／誰が／開始日時」で表示する。目標編集では改善ループの右側に最大ループ回数（1〜99、99は無制限）を表示する。草原の NPC（ネコ・イヌ・雲・蝶・飛行船）は `components/AIチーム/AIチーム_NPC制御.ts` が種別の登録と配置・更新の入口を持ち、個々の造形と動作は `AIチーム_NPC動作_ネコ.ts` / `_イヌ.ts` / `_雲.ts` / `_蝶.ts` / `_飛行船.ts` で調整する（ネコ・イヌ共通の四足処理は `AIチーム_NPC四足.ts`、共通型と擬似乱数は `AIチーム_NPC型.ts`）。NPC を増やすときは動作モジュールを追加し、`AIチーム_NPC制御.ts` の `NPC定義一覧` へ 1 行足す。チーム目標は NPC飛行船が 4 エリア上空を旋回しながら吊り下げる掲示板（常にカメラを向く）に最終更新値を表示し、クリックで `dialog/AIチーム_目標編集.vue`（`/team/目標/*`）を開く。掲示板の位置と向きは `AIチーム_NPC動作_飛行船.ts` の `吊り下げ物` として飛行船側が決める。
