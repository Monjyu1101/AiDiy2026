(() => {
  ...
  let wovenRails = []
  let lastLostTrail = []
  let evolutionMemory = createEvolutionMemory()
  let dualFlipLatched = false
  let weaveSerial = 0
  const requestedSeed = new URLSearchParams(location.search).get('seed')
  let randomSeed = normalizeSeed(requestedSeed ?? DEFAULT_SEED)
  let randomState = randomSeed
  let visualRandomState = randomSeed ^ 0x9e3779b9

  function normalizeSeed(value) {
    if (typeof value === 'number' && Number.isFinite(value)) return (Math.trunc(value) >>> 0) || DEFAULT_SEED
    const text = String(value ?? '').trim()
    if (/^[+-]?\d+$/.test(text)) return (Number(text) >>> 0) || DEFAULT_SEED
    let hash = 2166136261
  ...
  function nextRandom(stateName) {
    let state = stateName === 'game' ? randomState : visualRandomState
    state = (state + 0x6d2b79f5) >>> 0
    let value = state
    value = Math.imul(value ^ value >>> 15, value | 1)
    value ^= value + Math.imul(value ^ value >>> 7, value | 61)
    if (stateName === 'game') randomState = state
    else visualRandomState = state
    return ((value ^ value >>> 14) >>> 0) / 4294967296
  }

  function random(min, max) { return min + nextRandom('visual') * (max - min) }
  function gameRandom() { return nextRandom('game') }
  function resetDeterminism() {
    randomState = randomSeed
    visualRandomState = randomSeed ^ 0x9e3779b9
    time = 0
    simulationTick = 0
    accumulator = 0
  }
  function clamp(value, min, max) { return Math.max(min, Math.min(max, value)) }
  function createEvolutionMemory() {
    return {
      generation: 0, successes: [], failures: [], successPath: [], failurePath: [],
      dangerZones: [], rescuePath: [], rebound: 1, successfulHits: 0, rescues: 0
    }
  }
  function sampleTrail(trail, limit = 18) {
    if (!trail.length) return []
    if (trail.length <= limit) return trail.map((point) => [point.x, point.y])
    return Array.from({ length: limit }, (_, index) => {
      const point = trail[Math.round(index * (trail.length - 1) / (limit - 1))]
      return [point.x, point.y]
    })
  }
  function normalizeMemoryTrail(trail, bounds) {
    const valid = trail.filter((point) =>
      Number.isFinite(point.x) && Number.isFinite(point.y) &&
      point.x >= bounds.minX && point.x <= bounds.maxX &&
      point.y >= bounds.minY && point.y <= bounds.maxY
    )
    if (valid.length < 5) return []
    const oriented = valid[0].y < valid[valid.length - 1].y ? valid.slice().reverse() : valid.slice()
    const spaced = [oriented[0]]
    for (let index = 1; index < oriented.length - 1; index++) {
      const previous = spaced[spaced.length - 1]
      const point = oriented[index]
      if (Math.hypot(point.x - previous.x, point.y - previous.y) >= 18) spaced.push(point)
    }
    const lastPoint = oriented[oriented.length - 1]
    if (spaced[spaced.length - 1] !== lastPoint) spaced.push(lastPoint)
    return spaced.length >= 5 ? sampleTrail(spaced) : []
  }
  function removeClosePathPoints(points, minimumDistance = 18) {
    if (points.length < 2) return points.slice()
    const spaced = [points[0]]
    for (let index = 1; index < points.length - 1; index++) {
      const previous = spaced[spaced.length - 1]
      const point = points[index]
      if (Math.hypot(point[0] - previous[0], point[1] - previous[1]) >= minimumDistance) spaced.push(point)
    }
    const lastPoint = points[points.length - 1]
    if (spaced[spaced.length - 1] !== lastPoint) spaced.push(lastPoint)
    return spaced
  }
  function isEvolutionPhysicsAllowed(x, y) {
    if (x >= 540 && x <= 650 && y >= 360 && y <= 1040) return false
    if (x >= 520 && x <= 650 && y >= 72 && y <= 420) return false
    if (x >= 175 && x <= 545 && y >= 790 && y <= 960) return false
    if (x >= 280 && x <= 440 && y >= 930 && y <= 1060) return false
    return true
  }
  function makeEvolutionRail(memoryKind, points) {
    const rescue = memoryKind === 'rescue'
    return {
      id: ++weaveSerial, points, life: rescue ? 30 : 24, max: rescue ? 30 : 24,
      arm: rescue ? 0 : .2, active: !rescue, consumed: false,
      scar: rescue, evolved: true, memoryKind, hits: 0, scoreCooldowns: new Map()
    }
  }
  function mergeMemoryPaths(records, limit = 18) {
    if (!records.length) return []
    const newest = records[records.length - 1].points
    const previous = records.length > 1 ? records[records.length - 2].points : newest
    const length = Math.min(limit, newest.length, previous.length)
    return removeClosePathPoints(Array.from({ length }, (_, index) => {
      const previousIndex = Math.round(index * (previous.length - 1) / Math.max(1, length - 1))
      const newestIndex = Math.round(index * (newest.length - 1) / Math.max(1, length - 1))
      return [
        previous[previousIndex][0] * .35 + newest[newestIndex][0] * .65,
        previous[previousIndex][1] * .35 + newest[newestIndex][1] * .65
      ]
    }))
  }
  function rebuildEvolutionBoard(memoryKind) {
    evolutionMemory.successPath = mergeMemoryPaths(evolutionMemory.successes)
    evolutionMemory.failurePath = mergeMemoryPaths(evolutionMemory.failures)
    evolutionMemory.dangerZones = evolutionMemory.failures.slice(-4).map((record, index) => {
      const tail = record.points.slice(0, Math.min(5, record.points.length))
      return {
        x: tail.reduce((sum, point) => sum + point[0], 0) / tail.length,
        y: tail.reduce((sum, point) => sum + point[1], 0) / tail.length,
        r: 54 + index * 5, strength: .8 + index * .08
      }
    })
    evolutionMemory.rescuePath = removeClosePathPoints(evolutionMemory.failurePath
      .filter((point) => point[1] >= 652)
      .map((point, index, points) => [
        clamp(point[0] + (point[0] < W / 2 ? 34 : -34), 105, 540),
        clamp(point[1] - 42 - index / Math.max(1, points.length - 1) * 32, 610, 790)
      ]))
    evolutionMemory.rebound = clamp(1 + evolutionMemory.successes.length * .02 - evolutionMemory.failures.length * .005, 1.02, 1.12)
    evolutionMemory.generation++
    wovenRails = wovenRails.filter((rail) => !rail.evolved || rail.memoryKind !== memoryKind)
    if (memoryKind === 'success' && evolutionMemory.successPath.length >= 5) {
      wovenRails.push(makeEvolutionRail('success', evolutionMemory.successPath))
    }
    if (memoryKind === 'failure' && evolutionMemory.rescuePath.length >= 5) {
      wovenRails.push(makeEvolutionRail('rescue', evolutionMemory.rescuePath))
    }
    return memoryKind
  }
  function rememberSuccess(ball, weight = 1) {
    if (!ball || !ball.entered) return false
    ball.successMemory = (ball.successMemory || 0) + weight
    if (ball.successMemory < 2) return false
    const points = normalizeMemoryTrail(ball.trail, { minX: 105, maxX: 540, minY: 150, maxY: 790 })
    if (points.length < 5) return false
    evolutionMemory.successes.push({ points, weight: ball.successMemory })
    evolutionMemory.successes = evolutionMemory.successes.slice(-6)
    evolutionMemory.successfulHits++
    ball.successMemory = 0
    rebuildEvolutionBoard('success')
    return true
  }
  function rememberFailure(ball) {
    if (!ball || !ball.entered) return false
    const points = normalizeMemoryTrail(ball.trail, { minX: 105, maxX: 540, minY: 150, maxY: 1040 })
    if (points.length < 5) return false
    evolutionMemory.failures.push({ points })
    evolutionMemory.failures = evolutionMemory.failures.slice(-6)
    rebuildEvolutionBoard('failure')
    return true
  }
  ...
  function initStars() {
    ...
    stars = Array.from({ length: 90 }, () => ({
      x: random(0, W), y: random(0, H), z: random(.2, 1), phase: random(0, Math.PI * 2)
    }))
  }
  ...
  function noise(duration = .08, gain = .025) {
    ...
    if (!noiseBuffer) {
      ...
      for (let i = 0; i < data.length; i++) data[i] = random(-1, 1)
    }
  ...
  function makeBall(x = 584, y = 904, lane = true, vx = 0, vy = 0) {
    return { id: `${simulationTick}-${Math.trunc(gameRandom() * 0xFFFFFF)}`, x, y, vx, vy, r: 13, lane, entered: !lane, alive: true, born: time, trail: [], successMemory: 0, riftCooldown: 0, sailCooldown: 0, loomCooldown: 0, hue: gameRandom() > .5 ? '#ffdd75' : '#5cf8ff' }
  }

  function resetRound() {
    wovenRails.forEach((rail) => {
      if (rail.memoryKind === 'rescue' && !rail.consumed) {
        rail.active = true
        rail.life = rail.max = 30
        rail.arm = .2
      }
    })
    balls = [makeBall()]
  ...
  function startGame(useDemo = false) {
    ...
    loomEnergy = 0; wovenRails = []; lastLostTrail = []; evolutionMemory = createEvolutionMemory(); dualFlipLatched = false; weaveSerial = 0
  ...
  function weaveOrbit(force = false) {
    ...
    if (!rememberSuccess(source, 2)) return false
    const points = evolutionMemory.successPath
    loomEnergy = 0
  ...
  function preserveFailure() {
    ...
    const probeBall = { entered: true, trail: lastLostTrail }
    if (!rememberFailure(probeBall)) return
    const points = evolutionMemory.rescuePath
  ...
  function impact(x, y, color, power = 1) {
    ...
    noise(.04 + power * .02, .018 + power * .009); tone(170 + random(0, 250), .06, 'square', .025, 1.8)
  }
  ...
  function updateBall(ball, dt) {
    ...
    const activeRescueRail = wovenRails.find((rail) => rail.memoryKind === 'rescue' && rail.active && !rail.consumed && rail.life > 0)
    if (ball.entered && activeRescueRail && isEvolutionPhysicsAllowed(ball.x, ball.y)) for (const zone of evolutionMemory.dangerZones) {
      const dx = ball.x - zone.x; const dy = ball.y - zone.y; const distance = Math.hypot(dx, dy) || 1
      if (distance < zone.r) {
        const force = (1 - distance / zone.r) * zone.strength * 520 * dt
        ball.vx += dx / distance * force
        ball.vy -= Math.abs(dy / distance * force) + force * .45
      }
    }

    walls.forEach((wall) => segmentCollision(ball, wall[0], wall[1], 0, .9 * evolutionMemory.rebound))
    if (ball.entered && ball.loomCooldown <= 0 && isEvolutionPhysicsAllowed(ball.x, ball.y)) {
      for (const rail of wovenRails) {
        if (rail.arm > 0 || !rail.active || rail.consumed || rail.life <= 0) continue
        const scoreReadyAt = rail.scoreCooldowns.get(ball.id) || 0
        let collided = false
        const velocityBefore = ball.vy
        for (let index = 1; index < rail.points.length; index++) {
          const memoryBounce = rail.memoryKind === 'success' ? evolutionMemory.rebound : clamp(1.02 + evolutionMemory.failures.length * .012, 1.04, 1.1)
          if (segmentCollision(ball, rail.points[index - 1], rail.points[index], rail.scar ? 3 : 5, memoryBounce)) {
            collided = true
            break
          }
        }
        if (!collided) continue
        ball.loomCooldown = .18
        ball.vy -= rail.scar ? 115 + Math.min(evolutionMemory.failures.length, 6) * 10 : 145
        const validRescue = rail.scar && ball.vy < velocityBefore
        const validScore = time >= scoreReadyAt && (!rail.scar || validRescue)
        if (!validScore) break
        rail.scoreCooldowns.set(ball.id, time + .18)
        rail.hits++
        if (rail.scar) {
          evolutionMemory.rescues++
          rail.consumed = true
          rail.life = 0
        } else rememberSuccess(ball, 1)
        const color = rail.scar ? '#ff4f92' : '#5cf8ff'
        impact(ball.x, ball.y, color, rail.scar ? .7 : 1.15)
        addScore(rail.scar ? 160 : 420 + Math.min(rail.hits, 10) * 35, rail.scar ? 'FAILURE REMEMBERS' : 'SELF AUTHORED HIT', ball.x, ball.y - 18, color)
        break
      }
    }
  ...
  function update(dt) {
    ...
    wovenRails.forEach((rail) => {
      if (!rail.active || rail.consumed) return
      rail.arm = Math.max(0, rail.arm - dt)
      rail.life -= dt
    })
    wovenRails = wovenRails.filter((rail) => !rail.consumed && rail.life > 0)
  ...
  function frame(timestamp) {
    ...
    const elapsed = Math.max(0, (timestamp - last) / 1000)
    last = timestamp
    if (mode === 'play' || (demoMode && mode === 'over' && demoRestartAt > 0)) {
      accumulator = Math.min(MAX_ACCUMULATED_TIME, accumulator + elapsed)
  ...
  function togglePause(forceResume = false) {
    if (mode === 'play') { mode = 'paused'; ui.pause.classList.add('visible'); keys.left = keys.right = false }
      ...
      mode = 'play'; ui.pause.classList.remove('visible')
  ...
  window.XPinballSol = Object.freeze({
    getState: () => ({
      ...
      determinism: { seed: randomSeed, tick: simulationTick, timeStep: FIXED_TIME_STEP, accumulator, maxAccumulatedTime: MAX_ACCUMULATED_TIME },
      evolutionMemory: {
        generation: evolutionMemory.generation, successes: evolutionMemory.successes.length, failures: evolutionMemory.failures.length,
        successfulHits: evolutionMemory.successfulHits, rescues: evolutionMemory.rescues, rebound: evolutionMemory.rebound,
        successPath: evolutionMemory.successPath.length, failurePath: evolutionMemory.failurePath.length,
        rescuePath: evolutionMemory.rescuePath.length, dangerZones: evolutionMemory.dangerZones.map((zone) => ({ x: Math.round(zone.x), y: Math.round(zone.y), r: zone.r }))
      },
      wovenRails: wovenRails.map((rail) => ({ id: rail.id, scar: rail.scar, evolved: Boolean(rail.evolved), memoryKind: rail.memoryKind || null, life: rail.life, hits: rail.hits, points: rail.points.length })),
    ...
    setSeed: (seed) => {
      ...
      randomSeed = normalizeSeed(seed)
      resetDeterminism()
      initStars()
      return randomSeed
    },
    advanceTicks: (count, input = {}) => {
      if (mode !== 'paused') return false
      const ticks = clamp(Math.trunc(Number(count) || 0), 0, 120000)
      keys.left = Boolean(input.left)
      keys.right = Boolean(input.right)
      mode = 'play'
      for (let index = 0; index < ticks && mode === 'play'; index++) simulationStep()
