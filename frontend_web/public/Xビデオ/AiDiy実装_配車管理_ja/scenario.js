window.SCENARIO = {
  ...
  "scenes": [
    ...
    },
    {
      ...
      "long_narration": "T配車の編集フォームは、V配車一覧からも、S配車の週表示や日表示からも呼び出せます。既存の予定ブロックをダブルクリックすると編集、空いているセルをダブルクリックすると新規登録、という流れです。スケジューラーから開いた場合は、URLメニューとURL戻り先を持っているので、保存後に元の予定表へ戻れます。新規登録では、選んだ車両や日付、時間帯を引き継いでフォームを開けるため、ゼロから入力するより速く登録できます。車両と配車区分はマスタから選ぶ形式です。入力欄が日本語名でそろっているので、画面を見ながら「開始日時、終了日時、車両、内容、備考を入れる」と自然に理解できます。業務担当者と開発者が同じ言葉で話せる点も、AiDiyらしい使いやすさです。",
