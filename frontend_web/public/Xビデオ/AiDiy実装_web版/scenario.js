  ...
  "scenes": [
    {
      ...
      "facts": [],
      "evidence": [],
      "short_narration": "web版 AiDiy は、ブラウザで使う業務管理画面です。実画面で流れを順番に見ます。",
      "long_narration": "この動画では、AiDiy の web版を実際の画面で紹介します。web版は、ブラウザから使う業務管理システムの入り口です。画面は Vue 3、Vite、TypeScript で作られていて、ログインすると業務メニューや AI コアを操作できます。見た目は普通の管理画面ですが、中身は FastAPI のバックエンド、SQLite のデータベース、AI チャット、音声対話、コード支援までつながっています。最初に全体像を確認し、次にログイン、メニュー、AI コアのパネル、コード画面、ファイルや画像の扱い、最後に設定画面を順番に見ていきます。画面に映っている内容を追いながら、どこを触ると何ができるのかが分かるように説明します。",
    ...
    },
    {
      ...
      "long_narration": "ここは web版 AiDiy のログイン画面です。中央の小さなカードに、利用者 ID とパスワードを入力してログインします。裏側では FastAPI の認証 API に問い合わせ、正しければ JWT トークンが発行されます。JWT はログイン済みであることを示す通行証のようなもので、web版では localStorage に保存されます。画面から API を呼ぶときは Axios の共通クライアントがトークンを付けて送ります。もし期限切れなどで 401 エラーが返った場合は、自動的にログアウト処理へ進みます。利用者には権限コードがあり、管理者、リーダー、一般利用者、ゲストのように、見えるメニューや使える機能を分けられます。つまり、このシンプルなログイン画面が、業務データと AI 機能を安全に使い分ける入口になっています。",
    ...
    },
    {
      ...
      "long_narration": "ここまで web版 AiDiy の画面を見てきました。ログインして、メニューから業務画面を選び、必要に応じて AI コアを開き、チャット、コード、ファイル、画像、画面共有、設定を組み合わせて使います。Vue 3 と TypeScript の Web UI、FastAPI の API、SQLite のデータベース、日本語中心の命名、そして AI 支援がひとつにつながっています。配車、生産、在庫のような実装例を手本にすれば、別の業務にも応用できます。大切なのは、完成品をただ使うだけではなく、現場に合わせて変えられることです。小さなマスタ画面を追加するところから始めてもよいですし、AI コアを使ってコード修正を試してもよいです。AiDiy は、業務システムを自分たちで育てていくための土台です。あなたもAiDiyとシステムづくりしてみませんか？",
