...
METADATA_SYNC_PATH = BASE_DIR / "_sync_video_metadata.py"
...
def main() -> int:
    ...
    expected_ids = [*(f"scene_{number:03d}" for number in range(27)), "scene_999"]
    check(
        "シーン構成",
        scene_ids == expected_ids,
        f"{len(scene_ids)} シーン / 期待順序: scene_000～scene_026, scene_999",
    )
    check(
        "プロジェクト名",
    ...
    check("確認 2: PNG 枚数", len(image_files) >= 28, f"{len(image_files)} 枚（必要: 28 枚以上）")
    check("確認 3: MP3 個数", len(audio_files) >= 56, f"{len(audio_files)} 個（必要: 56 個以上）")
    ...
    for path in (AUDIO_GENERATOR_PATH, IMAGE_GENERATOR_PATH, METADATA_SYNC_PATH):
        if not path.is_file():
    ...
    check(
        "生成補助スクリプト構文",
        not script_errors,
        "3本とも正常" if not script_errors else "; ".join(script_errors),
    )
