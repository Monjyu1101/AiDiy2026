...
`aidiy_obs_studio_control` の接続情報は `_config/mcp_obs_studio_control.json` で管理する。
...
`aidiy_ffmpeg_control` の実行ファイルパスと既定タイムアウトは `_config/mcp_ffmpeg_control.json` で管理する。
...
- SQLite は `_data/AiDiy/database.db` を対象にする。
