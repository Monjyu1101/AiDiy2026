...
- VS Code に配置済みの `aidiy.aidiy-hermes` 拡張機能
- 各 `.venv` / `venv`
- `node_modules`
- `dist`
- `__pycache__`
...
- `frontend_vscode` の cleanup は `code --uninstall-extension` で拡張機能を解除し、拡張一覧から消えたことを確認してから依存関係と生成物を削除する。VS Code 本体は停止せず、解除の画面反映は利用者によるウィンドウ再読み込みに任せる。
