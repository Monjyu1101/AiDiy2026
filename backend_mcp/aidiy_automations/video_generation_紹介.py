...
"""
video_generation_紹介.py — 一人アバター（AiDiy）による紹介ビデオ自動生成スクリプト
...
◆ このスクリプトが作るビデオ:
    ...
    テンプレートは "AiDiy紹介_mcp" フォルダを使用。

◆ 対比: video_generation_解説.py
...
注意:
    - video_generation_setting.json の template_dir は "AiDiy紹介_mcp" 形式（version: "mcp"）の動画フォルダを前提にしている。
...
使い方:
    cd backend_mcp
    .venv\\Scripts\\python aidiy_automations\\video_generation_紹介.py [実行ステップ番号]

例:
    .venv\\Scripts\\python aidiy_automations\\video_generation_紹介.py

    .venv\\Scripts\\python aidiy_automations\\video_generation_紹介.py 04
"""
