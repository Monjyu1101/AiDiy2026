...
"""
video_generation_解説.py — 二人アバターによる掛け合い解説ビデオ自動生成スクリプト
...
◆ このスクリプトが作るビデオ:
    ...
    テンプレートは既存の "ニュース_*" フォルダを使用。

◆ 対比: video_generation_紹介.py
    一人アバター（AiDiy）が機能や仕組みを 1 体で紹介する紹介・ガイド型動画を生成する。
    scenario.js の version は "mcp"。

このスクリプトの目的:
    AiDiy の Xビデオ用テンプレートをもとに、二人アバター掛け合い解説型の動画素材を
...
主な機能:
    ...
    02. テーマ説明から scenario.js を生成する（version: "duo-v2"、dialogue 掛け合い形式）。
    ...
    06. scenario.js の dialogue.naration_text から audio/dlg_*.mp3 を生成する。
...
設計方針:
    ...
    - video_generation_steps.json に 99 が記録されたら、動画素材一式が完成した合図として処理を終了する。
    - 途中失敗しても、実行ステップ番号（00、01〜08、99）を指定して任意ステップだけ再実行できる。

注意:
    - video_generation_setting.json の template_dir は "duo-v2" 形式（二人掛け合い）の動画フォルダを前提にしている。
    - video_generation_setting.json の video_base_dir 配下に <フォルダ名> の出力フォルダを作成する。
    - video_generation_setting.json の folder_name と topic で題材と生成先を定義する。
    - Chrome 再描写の有無は video_generation_setting.json の browser_preview で指定する。
    - 画像生成と音声生成は MCP 側の image_generation / text_to_speech 実装に依存する。

使い方:
    cd backend_mcp
    .venv\\Scripts\\python aidiy_automations\\video_generation_解説.py [実行ステップ番号]

例:
    .venv\\Scripts\\python aidiy_automations\\video_generation_解説.py

    .venv\\Scripts\\python aidiy_automations\\video_generation_解説.py 04
"""
...
SETTING_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "video_generation_setting.json")

# 解説 / 紹介の実行完了ステップを共有管理する JSON。
STEPS_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "video_generation_steps.json")
...
class AutomationConfig:
    """video_generation.py の実行設定。設定 JSON と CLI 引数から組み立てる。"""
...
def load_setting_json() -> dict:
    """video_generation_setting.json を読み込んで返す。ファイルが無ければ空辞書。"""
    ...
    try:
        ...
        print(f"WARNING: video_generation_setting.json の読み込みに失敗しました: {e}")
        return {}


def save_steps_json(data: dict) -> None:
    """video_generation_steps.json を UTF-8 JSON として保存する。"""
...
def ensure_steps_json() -> dict:
    ...
    if os.path.isfile(STEPS_JSON_PATH):
        try:
            ...
            print(f"WARNING: video_generation_steps.json の読み込みに失敗しました。初期化します: {e}")
...
def require_mapping(data: dict, key: str) -> dict:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"video_generation_setting.json に {key} セクションが必要です")
    return value
...
def require_string(data: dict, key: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(f"video_generation_setting.json に文字列 {key} が必要です")
    return value
...
def require_bool(data: dict, key: str) -> bool:
    ...
    if not isinstance(value, bool):
        raise RuntimeError(f"video_generation_setting.json に bool {key} が必要です")
    return value
...
def require_int(data: dict, key: str) -> int:
    ...
    if not isinstance(value, int):
        raise RuntimeError(f"video_generation_setting.json に int {key} が必要です")
    return value


def build_config(argv: list[str]) -> AutomationConfig:
    """
    設定の優先度: 実行ステップCLI引数 > video_generation_steps.json / video_generation_setting.json

    OS 環境変数は使用しない。プログラム規定値は video_generation_setting.json に置く。
    """
...
def print_usage() -> None:
    print("使い方: python aidiy_automations/video_generation.py [実行ステップ番号]")
    print()
    print("  実行ステップ番号を指定した場合は、そのステップだけを実行します。")
    print("  引数を省略した場合は video_generation_steps.json の次ステップを実行します。")
    print("  OS 環境変数は使用しません。")
    print("  主な設定キー:")
    print("    topic                         題材、動画テーマ")
    print("    解説.folder_name              出力フォルダ名")
    print("    解説.template_dir             テンプレートフォルダ")
    ...
    print("  python aidiy_automations/video_generation.py 04")
...
async def step_create_scenario(
    ...
    prompt = (
        ...
        "■ 各シーンの推奨フィールド（あれば画像生成の品質が向上する）\n"
        '  "source_summary": そのシーンで表現したい内容の要約\n'
        '  "factual_bullets": 画像や台本に反映すべき具体要素を箇条書き\n'
        '  "forbidden_elements": 含めてはいけない表現や誇張\n'
...
async def step_mid_review(
    ...
    prompt = (
        ...
        "  2. scene_000 の最初の female 発話に、この動画が AiDiy の video_generation機能 で作られた旨が入っているか確認し、\n"
...
async def step_final_review(
    ...
    # ── 既完了チェック ──
    # video_generation_steps.json が 08 以上の場合、この最終確認は完了済みとして扱う。
    ...
    prompt = (
        ...
        f'    パス: "{gen_aud_py}"\n\n'
        "  確認 6: scene_000 の最初の female 発話に、この動画が AiDiy の video_generation機能 で作られた旨があるか\n"
        "  確認 7: 各 scene に必須フィールド（id, title, headline, image, dialogue）が揃っているか\n"
...
async def run_automation(config: AutomationConfig) -> None:
    ...
    if completed_step == "99" and not config.step_specified:
        message = (
            ...
            f"再実行する場合は video_generation_steps.json の {SCRIPT_TYPE} を空文字にしてください。"
        )
        print(f"\nvideo_generation_steps.json は {SCRIPT_TYPE}=99 です。完了済みとして終了します。")
