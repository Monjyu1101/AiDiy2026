...
主な機能:
    ...
    06. scenario.js の dialogue.naration_text から audio/*.mp3 を生成する。

    07. 生成済み audio/*.mp3 の実再生時間で scenario.js の duration_sec を更新する。

    08. 必須ファイル、画像数、音声数を最終確認し、成功時に 完成.txt を作成する。

    99. 完成.txt を確認し、完成案内だけを行う。
...
設計方針:
    ...
    - 途中失敗しても、開始ステップ番号（00、01〜08、99）を指定して任意ステップから再開できる。



注意:

    - AIDIY_VIDEO_GEN_TEMPLATE_DIR は既存のニュース型動画フォルダを前提にしている。

    - AIDIY_VIDEO_GEN_BASE_DIR 配下に <フォルダ名> の出力フォルダを作成する。

    - AIDIY_VIDEO_GEN_FOLDER_NAME と AIDIY_VIDEO_GEN_TOPIC で題材と生成先を定義できる。

    - AIDIY_VIDEO_GEN_BROWSER=0 で Chrome 再描写を無効化できる。
...
使い方:
    ...
    .venv\\Scripts\\python aidiy_automations\\video_generation.py [フォルダ名] [トピック詳細説明]



例:

    set AIDIY_VIDEO_GEN_FOLDER_NAME=ニュース_20260524_ms2026前半

    set AIDIY_VIDEO_GEN_TOPIC=Microsoft の 2026年前半ニュース解説

    .venv\\Scripts\\python aidiy_automations\\video_generation.py



    .venv\\Scripts\\python aidiy_automations\\video_generation.py ^

        "ニュース_20260524_ms2026前半" ^

        "Microsoft の 2026年前半ニュース解説。Azure AI、Copilot、GitHub Copilot 2.0 など"
...
# コピー元テンプレート。アバター一人語り（mcp形式）の index.html / scenario.js / 生成補助スクリプトを流用する。

DEFAULT_TEMPLATE_DIR = r"D:\OneDrive\_sandbox\AiDiy2026\frontend_web\public\Xビデオ\AiDiy紹介_mcp"



# 動画公開ディレクトリのルート。ここに <フォルダ名> の新規フォルダを作成する。

DEFAULT_VIDEO_BASE_DIR = r"D:\OneDrive\_sandbox\AiDiy2026\frontend_web\public\Xビデオ"

DEFAULT_FRONTEND_BASE_URL = "http://localhost:8090"

NEWS_VIDEO_KNOWLEDGE_PATH = os.path.join(REPO_DIR, ".aidiy", "knowledge", "backend_server,backend_mcp,MCP活用手順.md")

AUTO_VIDEO_KNOWLEDGE_PATH = os.path.join(REPO_DIR, ".aidiy", "knowledge", "共通,mcp利用による自動ビデオ生成手順.md")



# 環境変数未指定時の題材。CLI 引数なしでも自動化の流れを確認できるようにする。

DEFAULT_FOLDER_NAME = "AiDiy紹介_mcp改"

DEFAULT_TOPIC = "FastAPIとHTTP Transportを合わせたインターフェースをMCPハブに追加。これによりWebやpythonからのagent実行にも対応で、応用範囲が劇的に広がった点を強調した、13のMCPサーバー紹介ビデオ"

@dataclass

class AutomationConfig:

    """video_generation.py の実行設定。環境変数と CLI 引数から組み立てる。"""
    ...
    def kansei_path(self) -> str:

        return os.path.join(self.output_dir, "完成.txt")



# backend_mcp 専用 venv の Python。生成補助スクリプトはこの Python で実行する。

MCP_PYTHON = os.path.join(MCP_DIR, ".venv", "Scripts", "python.exe")



# 実行時設定。__main__ で環境変数と CLI 引数から作り、ここへ反映する。
...
def env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        print(f"WARNING: {name} は整数で指定してください。既定値 {default} を使います。")
...
def build_config(argv: list[str]) -> AutomationConfig:
    ...
    環境変数を基本設定として読み、CLI 引数があれば上書きする。
    ...
    環境変数:
        ...
        AIDIY_VIDEO_GEN_START_STEP   開始ステップ番号

        AIDIY_VIDEO_GEN_STOP_STEP    停止ステップ番号

        AIDIY_VIDEO_GEN_BASE_DIR     生成先ルートフォルダ

        AIDIY_VIDEO_GEN_TEMPLATE_DIR テンプレートフォルダ

        AIDIY_BACKUP_API_URL         差分バックアップ API

        AIDIY_TTS_API_URL            音声案内 API

        AIDIY_VIDEO_GEN_TTS          0/false/no/off で音声案内 OFF

        AIDIY_VIDEO_GEN_FRONTEND_URL frontend_web のベース URL

        AIDIY_VIDEO_GEN_BROWSER      0/false/no/off でブラウザ再描写 OFF

        CHROME_DEBUG_PORT            Chrome DevTools のデバッグポート

    """

    folder_name = os.environ.get("AIDIY_VIDEO_GEN_FOLDER_NAME", DEFAULT_FOLDER_NAME)

    topic = os.environ.get("AIDIY_VIDEO_GEN_TOPIC", DEFAULT_TOPIC)

    start_step = env_int("AIDIY_VIDEO_GEN_START_STEP", 0)

    stop_step = env_int("AIDIY_VIDEO_GEN_STOP_STEP", 99)
    ...
    if len(argv) >= 4:
        ...
        try:
            ...
            start_step = int(argv[3])

        except ValueError:

            print(f"ERROR: 開始ステップは整数で指定してください（指定値: {argv[3]}）")
    ...
    return AutomationConfig(
        ...
        max_retries=env_int("AIDIY_VIDEO_GEN_MAX_RETRIES", 3),

        retry_wait_sec=env_int("AIDIY_VIDEO_GEN_RETRY_WAIT_SEC", 5),

        max_backup_stabilize=env_int("AIDIY_VIDEO_GEN_MAX_BACKUP_STABILIZE", 5),
...
def print_usage() -> None:
    ...
    print("使い方: python aidiy_automations/video_generation.py [フォルダ名] [トピック詳細説明] [開始ステップ番号]")

    print()

    print("  CLI 引数を省略した場合は環境変数を使います。")

    print("  主な環境変数:")

    print("    AIDIY_VIDEO_GEN_FOLDER_NAME  出力フォルダ名")

    print("    AIDIY_VIDEO_GEN_TOPIC        題材、動画テーマ")

    print("    AIDIY_VIDEO_GEN_START_STEP   開始ステップ番号（00、01〜08、99）")

    print("    AIDIY_VIDEO_GEN_STOP_STEP    停止ステップ番号（00、01〜08、99）")

    print("    AIDIY_VIDEO_GEN_BASE_DIR     生成先ルートフォルダ")

    print("    AIDIY_VIDEO_GEN_TEMPLATE_DIR テンプレートフォルダ")

    print("    AIDIY_VIDEO_GEN_TTS          0 で音声案内 OFF")

    print("    AIDIY_VIDEO_GEN_BROWSER      0 で Chrome 再描写 OFF")

    print("    AIDIY_VIDEO_GEN_FRONTEND_URL frontend_web のベース URL")
    ...
    print('  python aidiy_automations/video_generation.py "ニュース_20260524_ms2026前半" "Microsoft 2026年前半ニュース解説"')

    print('  set AIDIY_VIDEO_GEN_FOLDER_NAME=ニュース_20260524_ms2026前半')

    print('  set AIDIY_VIDEO_GEN_TOPIC=Microsoft 2026年前半ニュース解説')

    print("  python aidiy_automations/video_generation.py")
...
def print_automation_flow(config: AutomationConfig) -> None:
    ...
    sep("AiDiy Automation: video_generation")
    ...
    print(f"開始ステップ   : {config.start_step}")

    print(f"停止ステップ   : {config.stop_step}")
...
async def refresh_browser_preview(
    ...
    *,
    ...
    aidiy_chrome_devtools 相当の CDP 実装で、生成中ビデオを ?auto=loop 表示する。
    ...
    try:
        ...
        manager = ChromeManager(debug_port=config.chrome_debug_port)

        await asyncio.to_thread(manager.ensure_running, False)
...
def render_dialogue_audio_script(output_dir: str) -> str:
    ...
    """Step 06 用の _gen_dialogue_audio.py 本文を返す。"""
    ...
    return textwrap.dedent(
        ...
        scenario.js の short_narration と long_narration から MP3 を生成します。
...
def run_python_script(script_path: str) -> None:
    ...
    """backend_mcp の Python で補助スクリプトを実行する。"""
...
async def probe_media_duration_sec(ffmpeg: FfmpegControl, media_path: str) -> float:

    """ffprobe でメディア再生時間を取得する。"""
    ...
    if result.returncode != 0:
        ...
        raise FfmpegControlError(f"ffprobe 失敗: {result.stderr[:300]}")
    ...
    try:
        ...
        raise FfmpegControlError(f"ffprobe の出力を数値化できません: {repr(raw)}") from e
...
async def update_scenario_audio_durations(scenario_path: str, new_dir: str) -> dict:
    ...
    """音声ファイルの実再生時間で scenario.js の duration_sec 群を更新する。"""
    ...
    for scene in data.get("scenes", []):
        ...
        for turn, dlg in enumerate(dialogues, start=1):
            ...
            duration_sec = await probe_media_duration_sec(ffmpeg, audio_path)
        ...
        scene["duration_sec"] = round(scene_duration_sec, 3)
    ...
    return {
        ...
        "dialogue_count": updated_dialogues,
...
def collect_scenario_duration_stats(scenario_path: str) -> dict:
    ...
    """scenario.js の再生時間欄が一貫しているか確認する。"""
    ...
    for scene in data.get("scenes", []):
        ...
        if abs(scene_dur - round(scene_sum, 3)) <= 0.01:
...
def count_scenario_dialogues(scenario_path: str) -> int:
    ...
    """scenario.js に含まれる音声ファイルの期待件数を返す。"""
    ...
    for scene in data.get("scenes", []):
        ...
        for dlg in scene.get("dialogue", []):
            ...
            if str(dlg.get("naration_text", "") or "").strip():
...
async def agent_run(ca: CodeAgents, prompt: str, timeout_sec: int = 300) -> str:

    """

    CodeAgents に作業指示を投げる共通ラッパー。
    ...
    使用 AI の選択は CodeAgents 側の共通ロジックへ委譲するが、
...
async def verify_and_backup_until_stable(
    ...
    for round_no in range(1, MAX_BACKUP_STABILIZE + 1):
        ...
        guide_tts(f"{step_name} の検証を開始します。{round_no}回目です。")
...
async def step00_preflight(
    ...
    環境変数 / CLI 引数から組み立てた設定、テンプレート、出力先、
    ...
    ok_agents = check("CodeAgents 利用可能", bool([k for k, v in ca.version_info.items() if v.get("ok")]))
...
async def step_create_scenario(
    ...
    prompt = (
        ...
        "  scene_000: イントロ（テーマ全体の概要、掛け合い 4〜5 発言）\n"

        "  scene_001〜006: 各テーマ（掛け合い 4〜5 発言ずつ）\n"

        "  scene_999: まとめ（掛け合い 4〜5 発言、最終発言は female）\n\n"

        "■ 各シーンの必須フィールド\n"

        '  "id", "title", "accent", "accent_soft", "kicker", "headline",\n'

        '  "image": "images/scene_NNN.png", "dialogue": [...],\n'

        '  "source_documents", "source_summary", "factual_bullets",\n'

        '  "forbidden_elements", "image_prompt"\n\n'

        "  - source_documents: 根拠にした資料名や記事名を短く列挙\n"

        "  - source_summary: そのシーンで表現したい事実の要約\n"

        "  - factual_bullets: 画像や台本に必ず反映すべき具体要素を箇条書き\n"

        "  - forbidden_elements: 入れてはいけない誤情報や誇張表現\n"

        "  - image_prompt: 上記要素を踏まえた画像生成向け説明（英語中心で可）\n\n"

        "■ 各 dialogue エントリの必須フィールド\n"

        '  "speaker": "female" または "male"\n'

        '  "expression": "neutral"\n'

        '  "telop_text": 字幕テキスト（短い要約。できれば 40〜60 文字程度）\n'

        '  "naration_text": 読み上げテキスト（200〜400文字）\n'

        '  "audio": "audio/dlg_NNN_NN_speaker.mp3"\n'

        '  "duration_sec": 20.0  （仮値でよい）\n\n'

        "  - telop_text と naration_text は同文コピペにせず、字幕は短く、読み上げは詳しく書く\n"

        "  - speaker の並びは female / male の掛け合いとして自然につながるようにする\n\n"

        "■ scene_000 の冒頭要件\n"

        "  - scene_000 の最初の dialogue は female にする\n"

        "  - 記事・ニュース内容の導入直後に\n"

        "    「このニュース解説動画は AiDiy のニュース版ビデオ生成機能で作られています」\n"

        "    という趣旨を naration_text に必ず入れる\n"

        "  - telop_text にも短い形で AiDiy 生成であることを反映する\n\n"

        "■ scene_999 最後の dialogue（female）に必ず含めること\n"
        ...
        "  - 断定が危うい情報は、発表・計画・見込み・報道ベースなど適切な言い回しにする\n"

        "  - 各 scene の source_summary / factual_bullets / forbidden_elements と、headline / dialogue / image_prompt を整合させる\n"

        "  - 画像生成で使えるよう、image_prompt には source_summary と factual_bullets の要点を反映する\n\n"
...
async def step_update_html(
    ...
    prompt = (
        ...
        "  - subtitle 表示は turn.telop_text を優先し、なければ subtitle / text にフォールバックする実装を残す。\n"

        "  - 音声読み上げ側は turn.naration_text を優先する実装を残す。\n"
...
async def step_generate_images(
    ...
    scenario.js からシーン情報を読み取り、ImageGeneration MCP 実装を使って
...
async def recover_mid_review_sources(
    ...
    *,
    ...
    if not await step_create_scenario(ca, backup_url, new_dir, folder_name, topic):

        return False

    if not await step_update_html(ca, backup_url, new_dir, folder_name, topic):

        return False

    if not await step_generate_images(ca, backup_url, new_dir, folder_name, topic):
...
async def step_mid_review(
    ...
    prompt = (
        ...
        "  2. scene_000 の最初の female 発話に、AiDiy のニュース版ビデオ生成機能で作られた旨が入っているか確認し、\n"

        "     telop_text にも短く反映されているか確認する。無ければ補う。\n"

        "  3. telop_text は短い字幕、naration_text は詳しい読み上げ、という役割分担になっているか確認する。\n"

        "     単純コピペなら telop_text を短く整える。\n"
        ...
        "  6. source_documents / source_summary / factual_bullets / forbidden_elements / image_prompt が scene ごとに揃い、\n"

        "     headline・dialogue・画像内容と矛盾しないか確認する。\n"

        "  7. index.html の title、brand、top-note、説明文も同じ観点で確認し、必要なら修正する。\n"

        "     2アバター、暗転、ステレオパン、字幕・音声フォールバックの実装を壊さない。\n"
...
async def step_generate_audio(
    ...
    _gen_dialogue_audio.py をこの Python から確定的に再生成し、その場で実行する。

    補助スクリプトは scenario.js の全 dialogue を読み取り、speaker に応じた

    音声ファイルを audio/dlg_<scene>_<turn>_<speaker>.mp3 として出力する。
    ...
    step_summary = (
        ...
        f'  "{folder_name}" の scenario.js から掛け合い音声を生成します。\n'

        "  _gen_dialogue_audio.py を作成・実行し、audio/dlg_*.mp3 を全 dialogue 件数ぶん揃えます。"
...
async def step_update_durations(
    ...
    dialogue.duration_sec を各 MP3 の実再生時間へ置き換え、scene.duration_sec と

    total_duration_sec も再計算する。音声が更新済みなら何度実行しても同じ結果になる。
    ...
    step_summary = (
        ...
        "  dialogue.duration_sec、scene.duration_sec、total_duration_sec を揃えます。"
    ...
    def validate() -> bool:
        ...
        ok1 = check(
            ...
            f"dialogue.duration_sec 更新数: {stats['dialogue_ok']}/{stats['dialogue_count']}",

            stats["dialogue_count"] > 0 and stats["dialogue_ok"] == stats["dialogue_count"],

        )

        ok2 = check(

            f"scene.duration_sec 整合数: {stats['scene_ok']}/{stats['scene_count']}",
        ...
        ok3 = check(
            ...
            "total_duration_sec 整合",

            stats["total_duration_sec"] > 0

            and abs(stats["total_duration_sec"] - stats["total_from_scenes"]) <= 0.01,
...
async def step_final_review(
    ...
    Step 08: 成果物一式を最終確認し、完成フラグを書き込む。



    scenario.js、画像、音声、index.html、生成補助スクリプトの存在を確認する。

    不足があれば CodeAgents に修正を依頼し、すべて OK のときだけ 完成.txt を

    Python 側で作成する。完成.txt は Step 99 の完成案内と main ループ側の

    終了条件に使う。

    """

    sep("Step 08: 最終確認・完成.txt 書き込み")
    ...
    step_summary = (
        ...
        "  scenario.js、index.html、images、audio、生成補助スクリプトを確認し、問題がなければ完成.txt を作成します。"

    )



    kansei_path   = os.path.join(new_dir, "完成.txt")
    ...
    # 完成.txt がある場合、このフォルダの最終確認は完了済みとして扱う。

    if os.path.isfile(kansei_path):

        print("  [SKIP] 完成.txt は既に存在します")
    ...
    prompt = (
        ...
        "  確認 5: _gen_dialogue_audio.py が存在するか\n"

        f'    パス: "{gen_aud_py}"\n\n'

        "  確認 6: scene_000 の最初の female 発話に、AiDiy のニュース版ビデオ生成機能で作られた旨があるか\n"

        "  確認 7: 各 scene に source_documents / source_summary / factual_bullets / forbidden_elements / image_prompt があるか\n"

        "  確認 8: index.html に字幕・音声フォールバック、暗転、ステレオパンの実装が残っているか\n\n"
    ...
    with open(kansei_path, "w", encoding="utf-8", newline="\n") as f:

        f.write(f"{folder_name}\n")

    mark_step_done(md_path, "完成")

    print(f"  [complete] 完成.txt を作成しました: {kansei_path}")
    ...
    if diff_count > 0:
        ...
        print("  [backup] 完成フラグ書き込み後の差分を保存します")
...
async def step_completion_notice(
    ...
    Step 08 で作成済みの 完成.txt を確認し、完了メッセージと音声案内だけを出す。
    ...
    if not os.path.isfile(kansei_path):

        print(f"  [NG] 完成.txt が見つかりません: {kansei_path}")
    ...
    with open(kansei_path, encoding="utf-8") as f:

        content = f.read().strip()

    print(f"  [complete] {step_name}: {content}")
...
async def run_automation(config: AutomationConfig) -> None:
    ...
    kansei_path = config.kansei_path
    ...
    if start_step not in valid_steps:
        ...
        print(f"ERROR: 開始ステップは 00、01〜08、99 で指定してください（指定値: {start_step}）")

        sys.exit(1)

    if stop_step not in valid_steps:

        print(f"ERROR: 停止ステップは 00、01〜08、99 で指定してください（指定値: {stop_step}）")

        sys.exit(1)

    if start_step > stop_step:

        print(f"ERROR: 開始ステップは停止ステップ以下で指定してください（開始={start_step}, 停止={stop_step}）")
    ...
    # Step 99 だけを明示指定した場合は、完成案内を再実行できるように通す。

    if os.path.isfile(kansei_path):

        if start_step >= 99:

            print("\n完成済みのため、Step 99 の完成案内だけを実行します。")
            ...
            print(f"\n完成.txt が既に存在します: {content}")

            guide_tts("完成ファイルが既に存在します。この動画生成は完了済みです。")
    ...
    # auto で利用できる AI CLI / Provider を確認し、利用可能なものがなければ中断する。
    ...
    for step_no, step_name, fn, args in steps:
        ...
        if step_no < start_step:
            ...
            print(f"\n[Step {step_no:02d}: {step_name}] SKIP（開始ステップ {start_step:02d} より前）")

            continue

        if step_no > stop_step:

            print(f"\n[Step {step_no:02d}: {step_name}] STOP（停止ステップ {stop_step:02d} より後）")

            break

        # 完成.txt が出来ていたら、通常処理は終了して Step 99 の完成案内へ進む。

        if os.path.isfile(kansei_path) and step_no < 99:
    ...
    if stop_step < 99:
        ...
        print(f"\n停止ステップ {stop_step:02d} までの検証を完了しました。")

        guide_tts(f"停止ステップ {stop_step:02d} までの検証を完了しました。")
    ...
    if os.path.isfile(kansei_path):
        ...
        print("\nERROR: 完成.txt が作成されませんでした")

        guide_tts("完成ファイルが作成されませんでした。処理を確認してください。", voice="male")
...
def main(argv: list[str] | None = None) -> None:
    ...
    # 1. 環境変数と CLI 引数から題材、生成先、API、リトライ条件を読み込む。
