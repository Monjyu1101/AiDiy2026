...
`backend_mcp` はポート `8095` 上で 14 個の MCP サーバーを同居させる FastMCP アプリケーションです。
ブラウザ操作、デスクトップキャプチャ、DB確認、ログ確認、コードチェック、バックアップ確認、画像/動画生成、音声認識/合成、OBS / ffmpeg 制御、コードエージェント実行を AI エージェントから利用できるようにします。
...
| パス | 役割 |
|------|------|
| `mcp_main.py` | FastAPI 上に 14 個の FastMCP を同居 |
| `mcp_stdio.py` | stdio <-> SSE bridge |
| `tests/` | MCP / HTTP API のテスト、動作確認スクリプト |
| `aidiy_automations/` | MCP / HTTP API を組み合わせる自動化スクリプト |
| `aidiy_automations/video_generation.py` | AiDiy 自動化ソリューション。Xビデオ用の自動ビデオ生成ワークフロー |
...
| `mcp_proc/backup.py` | 差分バックアップ保存 / 確認 |
| `mcp_proc/image_generation.py` | 画像生成（OpenAI / Gemini / FreeAI） |
| `mcp_proc/movie_generation.py` | 動画生成（Google Gemini Veo） |
...
| `mcp_proc/code_agents.py` | CodeAI CLI ラッパー（コードエージェント実行） |

## アーキテクチャ

### アクセスインターフェース（3種類）

| インターフェース | エンドポイント / コマンド | 主な利用者 |
|----------------|--------------------------|-----------|
| **SSE（MCP標準）** | `http://localhost:8095/{mcp_name}/sse` | AI エージェント、Claude Code、MCP クライアント |
| **stdio gateway** | `mcp_stdio.py --sse-url http://localhost:8095/{mcp_name}/sse` | Codex など stdio 専用の Code CLI |
| **HTTP POST（FastAPI）** | `POST http://localhost:8095/{mcp_name}/{method_name}` | Python スクリプト、curl、自動化処理 |

Swagger UI は `http://localhost:8095/docs` で全エンドポイントを確認・試行できます。
各 MCP の引数仕様は `GET http://localhost:8095/{mcp_name}/docs` が JSON で返します。
...
- `aidiy_automations/video_generation.py` は `localhost:8095/aidiy_text_to_speech/synthesize` の `play=true` を使い、進行案内を挟みながら実行する。
- `aidiy_automations/video_generation.py` の題材、生成先、テンプレート、開始ステップは `AIDIY_VIDEO_GEN_*` 環境変数で指定できる。`__main__` に全体の流れを置き、詳細処理は `step_*` 関数へ分ける。
- 自動化ステップは `00` を初期確認、`01`〜`nn` を実行と検証、`99` を最終処理として並べる。
- `aidiy_automations/video_generation.py` は `01`〜`99` の各ステップ完了後に、Chrome DevTools CDP で `?auto=loop` のビデオ表示を再描写する。
