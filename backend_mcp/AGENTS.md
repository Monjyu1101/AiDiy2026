...
`backend_mcp` はポート `8095` 上で 11 個の MCP サーバーを同居させる FastMCP アプリケーションです。
ブラウザ操作、デスクトップキャプチャ、DB確認、ログ確認、コードチェック、バックアップ確認、画像生成、音声認識/合成を AI エージェントから利用できるようにします。
...
| パス | 役割 |
|------|------|
| `mcp_main.py` | FastAPI 上に 11 個の FastMCP を同居 |
