...
`backend_mcp` はポート `8095` 上で 15 個の MCP サーバーを同居させる FastMCP アプリケーションです。
...
| パス | 役割 |
|------|------|
| `mcp_main.py` | FastAPI 上に 15 個の FastMCP を同居 |
...
- `aidiy_automations/video_generation.py` は `localhost:8095/tts` の `play=true` を使い、進行案内を挟みながら実行する。
