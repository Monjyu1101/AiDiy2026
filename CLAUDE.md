...
- **Backend MCP**: 14 個の MCP サーバー（Chrome, Desktop, SQLite, PostgreSQL, Logs, Code Check, Backup, Image Generation, Movie Generation, Speech-to-Text, Text-to-Speech, OBS Studio Control, FFmpeg Control, Code Agents）— SSE（MCP標準）/ stdio gateway（`mcp_stdio.py`）/ HTTP POST の 3 インターフェースを同一ポートで提供。ツール一覧は `GET http://localhost:8095/{mcp_name}/list`、Python からは `requests.post("http://localhost:8095/{mcp_name}/{method}")` で直接利用可能。
...
curl http://localhost:8095/tools/aidiy_code_check/check?path=backend_server/AIコア/AIコード_cli.py
```
...
curl -X POST http://localhost:8095/tools/aidiy_chrome_devtools/navigate \
