...
| Backend Hermes | Python CLI, prompt_toolkit, requests/openai, `aidiy_hermes` |
...
| Auth | JWT (python-jose, HS256, 60分、操作時延長あり) |
...
- **`npm run build` は明示依頼時のみ実行**（frontend_web・frontend_avatar 両方。dist を生成するため）
...
| キー | サフィックス | 例 |
|------|------------|-----|
| `CHAT_AI_NAME` | `_chat` で終わる | `gemini_chat`, `openrt_chat`, `freeai_chat`, `ollama_chat` |
| `LIVE_AI_NAME` | `_live` で終わる | `gemini_live`, `openai_live`, `freeai_live` |
| `CODE_AI1_NAME` 〜 `CODE_AI6_NAME` | `_sdk` または `_cli` で終わる（例外: `aidiy_hermes`） | `claude_sdk`, `claude_cli`, `copilot_cli`, `codex_cli`, `gemini_cli`, `opencode_cli`, `aidiy_hermes` |
...
**Ollama 固有の設定キー（`AiDiy_key.json`）：**
- `ollama_host` — Ollama サーバーの URL（デフォルト: `http://localhost:11434`）
- `CHAT_OLLAMA_MODEL` — 使用するモデル名（デフォルト: `deepseek-v4-flash:cloud`）
- モデル一覧は起動時に `ollama list` CLI で自動取得（`conf/conf_model.py` の `get_ollama_models()`）
...
# Backend Hermes (on-demand CLI)
cd backend_hermes && .venv/Scripts/python.exe cli_main.py
aidiy_hermes --help
...
- `_start.py` は `backend_hermes` を起動しない。`backend_hermes` は on-demand CLI として `aidiy_hermes` または `cli_main.py` を必要時に実行する。
...
加えて、**`backend_hermes`** が `aidiy_hermes` コマンドとして統合されている。これは HTTP サーバーではなく、AI コードパネルや手動操作から起動する **on-demand CLI 基盤** である。
...
**CORS 許可リスト（デフォルト）:** `http://localhost:8090`, `http://localhost:5173`, `http://localhost:3000`

### AIブラウザ自動操作（MCP連携）

- `backend_mcp/mcp_main.py` が 8 つの MCP サーバーを SSE で提供（ポート 8095）
...
- 認証延長: Web の C/M/T 操作系 API（`/core/C*`, `/core/V*`, `/apps/M*`, `/apps/T*`, `/apps/V*`）、S/V 更新監視、AI非音声入力（`input_text` / `input_file` / `input_image` / `input_request`）、AIファイル `files_temp` は送信時に `/core/auth/トークン更新` を呼ぶ
...
X系（Xテトリス、Xインベーダー、Xリバーシ、X立体リバーシ、X世界の絶景等）は Vue コンポーネントが **iframe ラッパー**になっており、実体は `frontend_web/public/X*/` 配下のスタンドアロン HTML+JS+CSS アプリ。
...
- Three.js 等の外部ライブラリは CDN から直接 import（Vite のバンドル対象外）
- X系ゲームを修正する場合は `public/X*/` 配下のファイルを直接編集する
...
- Electron ウィンドウ role 一覧: `login`, `core`（アバター常駐）, `chat`, `file`, `image`, `code1`〜`code6`, `settings`
...
自動テストスイートは設定されていない。テストは手動で行う：

- **API テスト**: FastAPI Swagger UI — http://localhost:8091/docs (Core) / http://localhost:8092/docs (Apps)
  - 「Authorize」ボタンで JWT トークンを入力してから各エンドポイントを実行
- **UI テスト**: ブラウザで http://localhost:8090 を直接操作
- **型チェック**: `cd frontend_web && npm run type-check` / `cd frontend_avatar && npm run type-check`
...
GitHub issue ページ（例: `https://github.com/monjyu1101/AiDiy2026/issues/1`）で本文・期待動作・最新コメントを確認してからローカル実装と突き合わせる。

### issue を close する（Windows Credential Manager 経由）

`gh` コマンドが使えない環境では PowerShell + GitHub REST API で操作する：

1. Windows Credential Manager から `GitHub - https://api.github.com/<GitHubユーザー名>` の credential を取得
2. `CredentialBlob` を **UTF-8** でデコードして token を得る（Unicode 解釈すると 401 になる）
3. close 前にコメントを投稿: `POST https://api.github.com/repos/monjyu1101/AiDiy2026/issues/<番号>/comments`
4. issue を close: `PATCH https://api.github.com/repos/monjyu1101/AiDiy2026/issues/<番号>` / body `{"state":"closed"}`
5. ヘッダー: `Authorization: Bearer <token>`
...
| CORS エラー | フロント URL が許可リストにない | `core_main.py` / `apps_main.py` の CORS 許可リストを更新（デフォルト: 8090, 5173, 3000） |
...
| X系ゲームを修正しても変わらない | Vue ファイルは iframe ラッパーのみ | `frontend_web/public/X*/` 配下のファイルを直接編集する |
