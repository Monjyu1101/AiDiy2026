...
| キー | サフィックス | 例 |
|------|------------|-----|
| `CHAT_AI_NAME` | `_chat` で終わる | `gemini_chat`, `openai_chat` |
| `LIVE_AI_NAME` | `_live` で終わる | `gemini_live`, `openai_live` |
| `CODE_AI1_NAME` 〜 `CODE_AI4_NAME` | `_code` で終わる | `claude_code`, `openai_code` |
...
- qTubler: カスタムテーブルコンポーネント（`_share/qTubler.vue`）— ソート・ページング・行選択付き
