...
**注意**: `_start.py` はバックエンドを `--reload` なしで起動します。コード変更を即反映したい場合は個別起動（上記コマンドは `--reload` 付き）を使うか、`backend_server/temp/reboot_core.txt` / `backend_server/temp/reboot_apps.txt` を作成して該当サーバーだけ再読込させます。

### 停止

```bash
# _start.py で起動した場合は Ctrl+C。_stop.py は存在しない。
# 手動停止する場合はポート使用プロセスを確認して kill
netstat -ano | findstr :8090   # Frontend Web
netstat -ano | findstr :8091   # Backend Core
netstat -ano | findstr :9098   # Backend Apps
netstat -ano | findstr :8093   # Backend Task
netstat -ano | findstr :8094   # Backend Local
netstat -ano | findstr :8095   # Backend MCP
netstat -ano | findstr :8092   # Frontend Avatar
taskkill /PID <pid> /F
```

`_start.py` 再実行時は対象ポートの既存プロセスを起動前に自動整理します。
