...
from 通信制御 import AIGuiConnector
from 画像制御 import 画像プレビューイベント, 画像送信制御
from 音声制御 import AudioOutputPlayer, MicrophoneInputStreamer
from log_config import get_logger
from util import AuthSession, GuiSettings, CoreEvent, システムアイドル秒数を取得

from .メインパネル import GUI表示バックエンド, 表示イベントコールバック, 表示バックエンドを生成
...
class DesktopGuiApp:
    ...
    def __init__(
        ...
        self.core_connector: AIGuiConnector | None = None
        ...
        self.display: GUI表示バックエンド = 表示バックエンドを生成(
    ...
    def _コア接続を開始(self) -> None:
        if self.skip_core_connect:
            return
        self.core_connector = AIGuiConnector(self.settings, self.auth_session, self.core_event_queue)
    ...
    def _GUI入力抑止を設定(self, enabled: bool) -> None:
        self.GUI入力抑止中 = enabled
        try:
    ...
    def _ドラッグ開始(self, event: tk.Event) -> None:
        if self.入力ダイアログ表示中 or self.GUI入力抑止中:
            return
    ...
    def _クリック解放(self, _event: tk.Event) -> None:
        if self.is_closing or self.入力ダイアログ表示中 or self.GUI入力抑止中:
            return
