...
from 通信制御.コア接続 import AIGuiConnector
...
class 画像送信制御:
    def __init__(self, auth_user_id: str, connector: AIGuiConnector, preview_queue: queue.Queue[画像プレビューイベント]) -> None:
    ...
    def 開始(self) -> bool:
        ...
        with self._lock:
            ...
            if self._thread is None or not self._thread.is_alive():
                self._thread = threading.Thread(target=self._ループを実行, name="gui-image-stream", daemon=True)
                self._thread.start()
        return True
