...
from tools.environments.local import _find_windows_shell, _sanitize_subprocess_env, _translate_posixish_to_powershell
...
def _local_shell_args(command: str) -> list[str]:
    shell_kind, shell_path = _find_windows_shell()
    if shell_kind == "powershell":
        script = "\n".join([
            "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8",
            "$OutputEncoding = [System.Text.Encoding]::UTF8",
            _translate_posixish_to_powershell(command),
            "exit $__hermes_ec",
        ])
        return [
            shell_path,
    ...
    return [shell_path, "-lc", f"set +m; {command}"]
...
class ProcessSession:
    ...
    _pty: Any = field(default=None, repr=False)  # winpty handle (when use_pty=True)
...
class ProcessRegistry:
    ...
    def spawn_local(
        ...
        Args:
            use_pty: If True, use a pseudo-terminal via winpty for interactive
                     CLI tools (Codex, Claude Code, Python REPL). Falls back to
                     subprocess.Popen if winpty is not installed.
        """
        ...
        if use_pty:
            ...
            try:
                ...
                logger.warning("winpty not installed, falling back to pipe mode")
            except Exception as e:
                logger.warning("PTY spawn failed (%s), falling back to pipe mode", e)

        # Standard Popen path (non-PTY or PTY fallback). Use the same local
        # shell selection as LocalEnvironment; on Windows this may be
        # PowerShell when Git Bash is unavailable.
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        if stdout is not None:
            logger.debug("Non-blocking pipe drain skipped on Windows for %s", session.id)
    ...
    def kill_process(self, session_id: str) -> dict:
        ...
        try:
            if session._pty:
                # PTY process -- terminate via winpty
                try:
                ...
                # Local process -- terminate the Windows child process
                try:
