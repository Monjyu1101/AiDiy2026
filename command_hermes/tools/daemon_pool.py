...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _adjust_thread_count(self) -> None:
        ...
        #
        # AiDiy runs this tree on Python 3.14, where CPython reworked the
        # worker signature: 3.8-3.13 pass ``(ref, work_queue, initializer,
        # initargs)`` while 3.14 passes ``(ref, ctx, work_queue)`` with ``ctx``
        # produced by ``self._create_worker_context()`` (``_initializer`` /
        # ``_initargs`` no longer exist as attributes).  Pick the argument
        # shape from the executor's own attributes so both runtimes work.
