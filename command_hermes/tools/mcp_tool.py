...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        # AiDiy MCP servers (SSE) are merged in automatically; a matching
        # entry in the Hermes config overrides them.
        try:
            aidiy_servers = _load_aidiy_mcp_servers()
            safe_servers.update(aidiy_servers)
            if aidiy_servers:
                logger.debug(
                    "Loaded %d AiDiy MCP server(s) from AiDiy_mcp.json",
                    len(aidiy_servers),
                )
        except Exception as exc:
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        # AiDiy: `AiDiy_mcp.json` は `type: "sse"` で SSE を指定する（接続側の
        # `_is_sse()` もそれを見る）。`transport` キーしか見ないと SSE サーバーが
        # バナーで "http" と表示され、障害切り分けを誤らせる。
        if "url" in cfg:
            transport = (
                "sse" if str(cfg.get("type", "")).lower() == "sse"
                else cfg.get("transport", "http")
            )
        else:
            transport = "stdio"
