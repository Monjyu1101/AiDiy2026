...
"""コマンド(hermes) セットアップスクリプト

このフォルダ (command_hermes) 単体のセットアップを行います。
...
def main():
    global AUTO_MODE
    print_header("コマンド(hermes) セットアップ")
    run_setup, AUTO_MODE = ask_start_mode("コマンド(hermes) のセットアップを実行しますか?", default="n")
    if not run_setup:
    ...
    ok = setup()
    if not ok:
        print_error("コマンド(hermes) のセットアップに失敗しました。")
        sys.exit(1)
