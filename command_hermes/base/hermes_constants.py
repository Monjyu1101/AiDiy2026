...
# AiDiy layout: this module lives in ``command_hermes/base``, so the install
# root is one level up from here (upstream keeps it at the repo root).
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent.parent / "scripts" / "lib" / "node-bootstrap.sh"
