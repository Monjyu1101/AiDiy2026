...
# AiDiy branding: the AIDIY-HERMES wordmark replaces upstream's HERMES-AGENT logo.
...
def format_banner_version_label() -> str:
    ...
    # AiDiy branding: the banner title carries the AiDiy name, not upstream's.
    base = f"AiDiy,Hermes v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # AiDiy: MCP サーバーは `_config/AiDiy_mcp.json` で管理し hermes の
    # config.yaml へは書かないので、上の cheap probe だけだと 19 サーバーが
    # まるごとバナーから消える（0.12 は無条件に get_mcp_status() を呼んでいた）。
    # ここでも同じローダーを見て、AiDiy 構成でもセクションが出るようにする。
    _has_aidiy_mcp = False
    if not (_has_native_mcp or _has_portable_mcp):
        try:
            from tools.mcp_tool import _load_aidiy_mcp_servers
            _has_aidiy_mcp = bool(_load_aidiy_mcp_servers())
        except Exception:
            _has_aidiy_mcp = True  # can't tell — take the full path
    if _has_native_mcp or _has_portable_mcp or _has_aidiy_mcp:
        try:
