...
def _resolve_use_tui(args) -> bool:
    ...
    """
    # AiDiy uses the Python prompt_toolkit TUI in cli_main.py. The upstream
    # Node/TypeScript TUI (``ui-tui/``) is intentionally not part of this
    # build, so never resolve to it — not even for an explicit ``--tui``.
    return False
