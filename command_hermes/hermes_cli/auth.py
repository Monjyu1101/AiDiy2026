...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    # Step 2: Show user the code and open the browser for them.
    #
    # AiDiy: upstream only prints the URL, which makes it easy to approve a
    # STALE code left over from an earlier attempt — the approval then binds
    # to a dead device_auth_id and this poll loop waits forever on HTTP 403.
    # Open the verification page automatically with the code pre-filled so
    # the code being approved is always the current one. Set
    # HERMES_NO_BROWSER=1 to keep the print-only behaviour (headless / SSH).
    from urllib.parse import quote as _quote

    device_url = f"{issuer}/codex/device"
    device_url_with_code = f"{device_url}?user_code={_quote(user_code)}"
    ...
    if os.getenv("HERMES_NO_BROWSER", "").strip().lower() not in {"1", "true", "yes"}:
        try:
            import webbrowser

            if webbrowser.open(device_url_with_code):
                print("Opened your browser with the code pre-filled.")
                print("(If nothing opened, use the URL and code above.)\n")
        except Exception as exc:
            logger.debug("Could not open browser for Codex device login: %s", exc)
