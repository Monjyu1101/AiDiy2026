...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    # AiDiy: `_config/AiDiy_mcp.json` の SSE サーバーを toolset 解決へ含める。
    #
    # AiDiy は MCP サーバー一覧を AiDiy_mcp.json だけで管理し（backend_tools/_setup.py が
    # 各 CLI へ書き出す唯一の情報源）、hermes の config.yaml へは書きません。
    # `tools/mcp_tool.py::_load_mcp_config()` は AiDiy パッチでこれをマージ済みなので
    # MCP ランタイムには接続されますが、この関数がそれを見ないと
    # `_get_platform_tools()` が `mcp-aidiy_*` toolset を有効にせず、
    # 214 個のツールがモデルへ一切届きません（登録はされているのに使えない）。
    # 情報源を増やさずに揃えるため、ここでも同じローダーを使います。
    try:
        from tools.mcp_tool import _load_aidiy_mcp_servers

        names |= {
            str(name) for name in _load_aidiy_mcp_servers()
        } - set(mcp_servers)
    except Exception:
        logger.debug("Failed to include AiDiy MCP servers", exc_info=True)
    return names
