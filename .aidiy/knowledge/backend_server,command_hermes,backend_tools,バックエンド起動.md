...
> 文書: `backend_server,command_hermes,backend_tools,バックエンド起動.md` | 実装: `_start.py`, `backend_server/temp/reboot_core.txt`, `backend_task/task_main.py`

## このメモを使う場面
- バックエンドのコード変更を反映させる
- core/apps/tools/task を起動または再起動する
...
- `backend_task/temp/reboot_task.txt` — task 再起動トリガー
- `backend_task/task_main.py` — task サーバー本体
...
- `backend_task` は `_start.py` では apps 起動後に起動する。現在の定期タスクは 60 秒間隔で `task done !, HH:MM:SS` をログ表示する。`backend_task/temp/reboot_task.txt` 検知で終了し、`_start.py` 起動中なら自動再起動される。
