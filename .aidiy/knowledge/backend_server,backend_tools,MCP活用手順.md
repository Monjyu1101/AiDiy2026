...
- `backend_tools/mcp_main.py` — 14個の SSE MCP サーバー入口
- `backend_tools/mcp_stdio.py` — SSE を stdio client へ中継
- `backend_tools/mcp_proc/` — 各 MCP のロジック
...
- 手動起動は `cd backend_tools && .venv/Scripts/python.exe -m uvicorn mcp_main:app --reload --host 0.0.0.0 --port 8095`
