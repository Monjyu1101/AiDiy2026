...
## このメモを使う場面
- Claude / Gemini / Codex などの Code CLI から AiDiy MCP を使わせる
...
## Codex から AiDiy MCP を使う設定

Codex の `url = ...` は streamable HTTP 用で、AiDiy の SSE endpoint を直接扱えない。`backend_mcp/mcp_stdio.py` を挟む。
...
他の AiDiy MCP も `--sse-url` の URL を `/aidiy_sqlite/sse` などへ変えて登録する。

## セットアップ/クリーンアップの方針

- `python _setup.py` は必要に応じて Claude / Gemini / Copilot / OpenCode / Codex 用 MCP 設定を生成する
...
- Codex から AiDiy MCP へつなぐ場合は、`backend_mcp` と対象 SSE サーバーが起動済みであることを先に確認する
...
`_setup.py` 実行後は `.gitignore` に `.claude/` と `.gemini/` が含まれること、Codex 設定では `mcp_stdio.py --sse-url ...` 形式になっていることを確認する。
