...
def cleanup(choices: dict) -> None:
    ...
    deleted_count = cleanup_common_python_caches(BACKEND_TASKTEAM_DIR, label)

    taskteam_envs = choices.get("taskteam_envs", {})
    for env_name in BACKEND_TASKTEAM_ENV_LIST:
        env_dir = BACKEND_TASKTEAM_DIR / env_name
        if not env_dir.exists():
            continue
        if taskteam_envs.get(env_name):
            if remove_directory(env_dir, f"{env_name} ({label})"):
                deleted_count += 1
        elif env_name in taskteam_envs:
            print_info(f"  {env_name} はそのまま残します")

    temp_dir = BACKEND_TASKTEAM_DIR / "temp"
    if temp_dir.exists():
        if choices.get("taskteam_temp") is True:
            if remove_directory(temp_dir, f"temp ({label})"):
                deleted_count += 1
        elif choices.get("taskteam_temp") is False:
...
def collect_choices() -> dict:
    choices: dict = {"taskteam_envs": {}, "taskteam_temp": None}
    if BACKEND_TASKTEAM_DIR.exists():
        for env_name in BACKEND_TASKTEAM_ENV_LIST:
            if (BACKEND_TASKTEAM_DIR / env_name).exists():
                choices["taskteam_envs"][env_name] = ask_yes_no(f"  {env_name} を削除しますか？", default="y")
        if (BACKEND_TASKTEAM_DIR / "temp").exists():
            choices["taskteam_temp"] = ask_yes_no("  temp フォルダを削除しますか？", default="y")
    return choices


def main() -> None:
    global AUTO_MODE
    print_header("バックエンド(task,team) クリーンアップ")
