...
class タスク停止復旧リクエスト(BaseModel):
    ...
    強制=False（既定）はエラーが無ければ何もしない。監視タスクが実行中の
    タスクを誤って巻き戻さないための安全弁。
    """
    タスクID: str
    強制: bool = False
...
async def タスク要求停止検査(request: タスク停止検査リクエスト) -> dict:
    ...
    1〜4 は /タスク要求/停止復旧 で復旧できる（復旧可能=true）。
    5 は実行中扱いのままなので、監視ループの打ち切りを待つか 強制=true が要る。
    ...
    try:
        ...
        if タスクID and not items and not request.停止のみ:
            return _NG(f"タスク {タスクID} が見つかりません。")
...
async def タスク要求停止復旧(request: タスク停止復旧リクエスト) -> dict:
    ...
    エラーの要求・明細だけを 待機 / 実行有効=1 に戻し、完了済みの明細は残す。
    そのため再開は最初からではなく「止まったところから」になる。

    応答の data には復旧前後の状態を入れる。復旧前.エラー明細 は応答内容つきなので、
    監視タスクはこの 1 回の呼び出しだけで原因分析と復旧結果の確認まで済ませられる。

    強制=False（既定）でエラーが無い場合は何もせず 復旧実施=false を返す。
    """
    タスクID = request.タスクID.strip()
    if not タスクID:
        return _NG("タスクIDを指定してください。")
    try:
        結果 = tasks_db.タスク停止復旧(タスクID, request.強制)
        ...
        if 結果["復旧実施"]:
            ...
            message = (
                f"タスク {タスクID} を復旧しました"
                f"（エラー明細 {件数} 件を待機へ、再開SEQ={結果['再開SEQ']}）。"
            )
        else:
...
async def タスク明細再試行(request: タスク明細再試行リクエスト) -> dict:
    ...
    try:
        ...
        if not タスクID:
            return _NG("タスクIDを指定してください。")
        結果 = tasks_db.明細再試行(タスクID, request.明細SEQ)
        予測分数 = 結果["予測分数"]
        return _OK(
