...
DB_PATH = (PROJECT_ROOT / "backend_server" / DEFAULT_DATABASE_PATH).resolve()
要員テーブル = "Aチーム要員"
管理者要員ID = "admin"
...
def 初期化() -> None:
    """テーブル作成を行う。多重呼び出し可。"""
    # CREATE TABLE / CREATE TRIGGER は IF NOT EXISTS でも書き込みロックを取るため、
    # 毎回呼ぶと "database is locked" を招く。プロセス内で一度成功したら以降は何もしない。
    global _初期化済み
    if _初期化済み:
        return
    conn = 接続取得()
