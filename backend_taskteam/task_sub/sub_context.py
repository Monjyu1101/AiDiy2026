...
_DO_VERIFY_TEMPLATE_LINES = [
    ...
    "  Body: {\"タスクID\": \"{タスクID}\", \"SEQ\": {明細SEQ}, \"状態\": \"完了\", \"メッセージ\": \"実行内容と検証結果の報告\"}",
    "  検証で問題が見つかった場合は 状態 を \"エラー\" にし、メッセージ に理由を書いてください。",
    "  タスクID と SEQ は上のとおりに指定してください。作業の中で参照した別のタスクIDへ報告してはいけません。",
    "  メッセージは空にできません（空の完了報告は API に拒否されます）。実行した内容、確認した値、",
    "  結果を、後続ステップと最終検証がこの本文だけを読んで判断できる詳しさで書いてください。",
    "  この本文がそのまま実行済み記録として残ります。応答本文へ書いただけでは記録されません。",
]
...
_CHECK_REQUEST_TEMPLATE_LINES = [
    ...
    "  POST http://127.0.0.1:8093/task_check_okng",
    "  Content-Type: application/json",
    "  Body: {\"タスクID\": \"{タスクID}\", \"SEQ\": {明細SEQ}, \"状態\": \"完了\", \"メッセージ\": \"検証結論の報告\"}",
    "  問題が見つかった場合は 状態 を \"エラー\" にし、メッセージ に理由を書いてください。",
    "  タスクID と SEQ は上のとおりに指定してください。検証の中で参照した別のタスクIDへ報告してはいけません。",
    "  メッセージは空にできません（空の完了報告は API に拒否されます）。検証した対象と結論を本文に書いてください。",
