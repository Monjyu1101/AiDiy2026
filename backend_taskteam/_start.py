...
"""バックエンド(task,team) 起動スクリプト。

AIタスク実行とAIチームを統合した FastAPI サーバーをポート 8093 で起動します。
...
BACKEND_TASKTEAM_DIR = THIS_DIR
PORT_TASKTEAM = 8093
APP = "taskteam_main:app"
...
def get_command() -> list[str]:
    python_path = find_python_in_env(BACKEND_TASKTEAM_DIR, ENV_CANDIDATES)
    if python_path is not None:
        return [str(python_path), "-m", "uvicorn", APP, "--host", "0.0.0.0", "--port", str(PORT_TASKTEAM)]
    return ["uv", "run", "uvicorn", APP, "--host", "0.0.0.0", "--port", str(PORT_TASKTEAM)]


def check_environment() -> tuple[bool, str]:
    if not BACKEND_TASKTEAM_DIR.exists():
        return False, f"フォルダが見つかりません: {BACKEND_TASKTEAM_DIR}"
    if not (BACKEND_TASKTEAM_DIR / "pyproject.toml").exists():
        return False, f"pyproject.toml が見つかりません: {BACKEND_TASKTEAM_DIR / 'pyproject.toml'}"
    if not (BACKEND_TASKTEAM_DIR / "taskteam_main.py").exists():
        return False, f"taskteam_main.py が見つかりません: {BACKEND_TASKTEAM_DIR / 'taskteam_main.py'}"
    python_path = find_python_in_env(BACKEND_TASKTEAM_DIR, ENV_CANDIDATES)
    if python_path is not None:
...
def start() -> subprocess.Popen[bytes]:
    return launch_process("バックエンド(task,team)", get_command(), BACKEND_TASKTEAM_DIR)
...
def kill_ports() -> None:
    kill_process_on_port(PORT_TASKTEAM)
...
def main() -> None:
    ...
    print_success(f"{name}: http://127.0.0.1:{PORT_TASKTEAM}/docs")
