...
設計方針:
    ...
    - ビデオ生成__状況.json に 99 が記録されたら、動画素材一式が完成した合図として処理を終了する。
    - 途中失敗しても、実行ステップ番号（00、01〜08、99）を指定して任意ステップだけ再実行できる。

注意:
    - ビデオ生成__設定.json の template_dir は "duo-v2" 形式（二人掛け合い）の動画フォルダを前提にしている。
    - ビデオ生成__設定.json の video_base_dir 配下に <フォルダ名> の出力フォルダを作成する。
    - ビデオ生成__設定.json の folder_name と topic で題材と生成先を定義する。
    - Chrome 再描写の有無は ビデオ生成__設定.json の browser_preview で指定する。
...
SETTING_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ビデオ生成__設定.json")

# 解説 / 紹介の実行完了ステップを共有管理する JSON。
STEPS_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ビデオ生成__状況.json")
...
def load_setting_json() -> dict:
    """ビデオ生成__設定.json を読み込んで返す。ファイルが無ければ空辞書。"""
    ...
    try:
        ...
        print(f"WARNING: ビデオ生成__設定.json の読み込みに失敗しました: {e}")
        return {}


def save_steps_json(data: dict) -> None:
    """ビデオ生成__状況.json を UTF-8 JSON として保存する。"""
...
def ensure_steps_json() -> dict:
    ...
    if os.path.isfile(STEPS_JSON_PATH):
        try:
            ...
            print(f"WARNING: ビデオ生成__状況.json の読み込みに失敗しました。初期化します: {e}")
...
def require_mapping(data: dict, key: str) -> dict:
    ...
    if not isinstance(value, dict):
        raise RuntimeError(f"ビデオ生成__設定.json に {key} セクションが必要です")
    return value
...
def require_string(data: dict, key: str) -> str:
    ...
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(f"ビデオ生成__設定.json に文字列 {key} が必要です")
    return value
...
def require_bool(data: dict, key: str) -> bool:
    ...
    if not isinstance(value, bool):
        raise RuntimeError(f"ビデオ生成__設定.json に bool {key} が必要です")
    return value
...
def require_int(data: dict, key: str) -> int:
    ...
    if not isinstance(value, int):
        raise RuntimeError(f"ビデオ生成__設定.json に int {key} が必要です")
    return value


def build_config(argv: list[str]) -> AutomationConfig:
    """
    設定の優先度: 実行ステップCLI引数 > ビデオ生成__状況.json / ビデオ生成__設定.json

    OS 環境変数は使用しない。プログラム規定値は ビデオ生成__設定.json に置く。
    """
...
def print_usage() -> None:
    ...
    print("  引数を省略した場合は ビデオ生成__状況.json の次ステップを実行します。")
...
async def step_final_review(
    ...
    # ── 既完了チェック ──
    # ビデオ生成__状況.json が 08 以上の場合、この最終確認は完了済みとして扱う。
...
async def run_automation(config: AutomationConfig) -> None:
    ...
    if completed_step == "99" and not config.step_specified:
        message = (
            ...
            f"再実行する場合は ビデオ生成__状況.json の {SCRIPT_TYPE} を空文字にしてください。"
        )
        print(f"\nビデオ生成__状況.json は {SCRIPT_TYPE}=99 です。完了済みとして終了します。")
