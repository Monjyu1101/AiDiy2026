...
ビデオページ生成/
  ...
  ビデオページ生成_小説小話.py     小説・小話型動画の生成
  ビデオページ生成_翻訳ja2xx.py    既存ビデオの多言語（ja → 他言語）展開
  無限解説ビデオ生成.py            解説スクリプトを外側から継続実行させるランナー
  utils/                           各スクリプト共通処理（ctx / infra / steps / generation / runner / log_config）
  _ビデオページ生成_<種別>_設定.json  種別ごとの専用設定
  _ビデオページ生成_<種別>_状況.json  種別ごとの実行完了ステップ管理（初回実行時に生成）
```

`<種別>` は `紹介` / `解説` / `小説小話` / `翻訳ja2xx` です。
...
# 解説ビデオを題材更新込みで回し続ける
.venv/Scripts/python aidiy_automations/ビデオページ生成/無限解説ビデオ生成.py
```
