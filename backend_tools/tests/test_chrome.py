...
ツールを呼び出す。事前に backend_tools (port 8095) が起動済みであること。
...
backend_tools_DIR = Path(__file__).resolve().parent.parent

# backend_tools をパスに追加
sys.path.insert(0, str(backend_tools_DIR))

from mcp.client.session import ClientSession
from mcp.client.stdio import StdioServerParameters, stdio_client

PYTHON     = str(backend_tools_DIR / ".venv" / "Scripts" / "python.exe")
STDIO_SCRIPT = str(backend_tools_DIR / "mcp_stdio.py")
