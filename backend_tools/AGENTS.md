...
`backend_tools` はポート `8095` 上で 16 個の MCP サーバーを同居させる FastMCP アプリケーションです。
ブラウザ操作、デスクトップキャプチャ、DB確認、ログ確認、コードチェック、バックアップ確認、画像/動画生成、音声認識/合成、OBS / ffmpeg 制御、通知音再生、コードエージェント実行、チャット LLM 実行を AI エージェントから利用できるようにします。
