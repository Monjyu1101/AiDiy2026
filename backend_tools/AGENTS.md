...
`backend_tools` はポート `8095` 上で 17 個の MCP サーバーを同居させる FastMCP アプリケーションです。
ブラウザ操作、デスクトップキャプチャ、DB確認、ログ確認、コードチェック、バックアップ確認、画像/動画生成、音声認識/合成、OBS / ffmpeg 制御、通知音再生、コードエージェント実行、チャット LLM 実行、AIタスク非同期投入を AI エージェントから利用できるようにします。
...
| `aidiy_task_agents` | backend_task API への AIタスク非同期投入 |
...
| パス | 役割 |
|------|------|
| `tools_main.py` | FastAPI 上に 17 個の FastMCP を同居 |
