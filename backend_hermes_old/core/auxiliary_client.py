...
from base.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        if temperature is not None:
            from core.anthropic_adapter import _forbids_sampling_params
...
def _maybe_wrap_anthropic(
    ...
    if isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from core.gemini_native_adapter import GeminiNativeClient
        if isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from core.copilot_acp_client import CopilotACPClient
    ...
    try:
        from core.anthropic_adapter import build_anthropic_client
    except ImportError:
        logger.warning(
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from core.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from core.nous_rate_guard import nous_rate_limit_remaining
...
def _try_anthropic() -> Tuple[Optional[Any], Optional[str]]:
    try:
        from core.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
    ...
    from core.anthropic_adapter import _is_oauth_token
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from core.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from core.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from core.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
            from core.anthropic_adapter import build_anthropic_bedrock_client
        except ImportError:
