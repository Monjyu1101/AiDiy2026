...
def 引数を解析() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AiDiy desktop avatar")
    parser.add_argument(
        "--settings",
        type=Path,
    ...
    parser.add_argument(
        "--skip-login",
        action="store_true",
        help="start avatar without authentication dialog",
    )
    parser.add_argument(
        "--skip-core-connect",
        action="store_true",
        help="start avatar without AI core websocket connections",
    )
...
def 起動() -> int:
    ...
    try:
        ...
        return 1
    app.実行()
    logger.info("frontend_avatar は正常終了しました")
    return 0
