...
| `CODE_AI1_NAME` 〜 `CODE_AI4_NAME` | 原則 `_sdk` または `_cli`、例外 `aidiy_hermes` | `claude_sdk` / `claude_cli` / `copilot_cli` / `codex_cli` / `gemini_cli` / `opencode_cli` / `aidiy_hermes` |
...
- `CHAT_AI_NAME` は `_chat` 末尾、`LIVE_AI_NAME` は `_live` 末尾、`CODE_AI*_NAME` は原則 `_sdk` または `_cli`、例外 `aidiy_hermes` と `opencode_cli`
