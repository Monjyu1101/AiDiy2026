...
- 左ペイン: アバター（`AIコア.vue`）
- 右ペイン: タブバー＋パネルエリア（`chat` / `file` / `image` / `code1〜6` をタブ切替）
...
- `CHAT_AI_NAME` は `_chat` 末尾、`LIVE_AI_NAME` は `_live` 末尾、`CODE_AI*_NAME` は原則 `_sdk` または `_cli`、例外 `aidiy_hermes`（`opencode_cli` は `_cli` 末尾の通常枠）
