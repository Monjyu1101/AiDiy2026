...
import AIタスクビュー from '@/components/AIタスク/AIタスク.vue'
import AIタスク要求ウィンドウ from '@/components/AIタスク/windows/AIタスク_要求ウィンドウ.vue'
import AIタスクフローウィンドウ from '@/components/AIタスク/windows/AIタスク_フローウィンドウ.vue'
...
import AIチームビュー from '@/components/AIチーム/AIチーム.vue'
...
type WindowRole = 'login' | 'core' | PanelKey | TaskKey | 'settings' | 'taskDialog' | 'team'
type チャットモード型 = 'chat' | 'live' | 'code1' | 'code2' | 'code3' | 'code4' | 'code5' | 'code6'
type コードチャンネル = '1' | '2' | '3' | '4' | '5' | '6'
type 右ペインタブ型 = 'core' | 'image' | 'code1' | 'code3' | 'code5'
type SharedSnapshot = {
  認証済み: boolean
  利用者ラベル: string
  セッションID: string
...
const PANEL_TITLES = computed<Record<PanelKey, string>>(() => {
  ...
  return {
    ...
    code5: t(5, s.CODE_AI5_NAME),
    code6: t(6, s.CODE_AI6_NAME),
  }
})
const チャットモード一覧: チャットモード型[] = ['chat', 'live', 'code1', 'code2', 'code3', 'code4', 'code5', 'code6']
...
function フォールバックロール解決(): WindowRole {
  ...
  if (
    role === 'core' || role === 'login' || role === 'settings' || role === 'team'
    || role === 'taskDialog'
    || PANEL_KEYS.includes(role as PanelKey)
    || TASK_KEYS.includes(role as TaskKey)
  ) {
...
const コード5Ref = ref<コードパネル参照型 | null>(null)
const コード6Ref = ref<コードパネル参照型 | null>(null)
...
// AIチームはWebでは同一画面内、ElectronではAvatar rendererの専用ウィンドウで表示する
const AIチーム表示 = ref(false)
const 分割比率 = ref(50)
const リサイズ中 = ref(false)

async function チーム画面切替() {
  if (isElectron) {
    await window.desktopApi?.toggleTeamWindow?.()
    return
  }

  AIタスク表示.value = false
  AIチーム表示.value = !AIチーム表示.value
}
...
const webTabs: { key: PanelKey; label: string }[] = [
  ...
  { key: 'code6', label: 'コード6' },
]

const webRightTabs: { key: 右ペインタブ型; label: string }[] = [
  { key: 'core', label: 'コア' },
  { key: 'image', label: 'イメージ' },
  { key: 'code1', label: 'コード1' },
  { key: 'code3', label: 'コード3' },
  { key: 'code5', label: 'コード5' },
]
...
const WebアクティブコードRef = computed<コードパネル参照型 | null>(() => {
  switch (アクティブタブ.value) {
    ...
    case 'code6':
      return コード6Ref.value
    default:
      return null
  }
})

const Web右アクティブコードRef = computed<コードパネル参照型 | null>(() => {
  switch (右アクティブタブ.value) {
    case 'code1':
      return コード1Ref.value
    case 'code3':
      return コード3Ref.value
    case 'code5':
      return コード5Ref.value
    default:
      return null
  }
})
...
const Web右ペインタイトル = computed(() => {
  if (右アクティブタブ.value === 'code1' || 右アクティブタブ.value === 'code3' || 右アクティブタブ.value === 'code5') {
    return PANEL_TITLES.value[右アクティブタブ.value]
  }
  return PANEL_TITLES.value.image
})

const Web右ペイン状態表示 = computed(() => {
  if (右アクティブタブ.value === 'code1' || 右アクティブタブ.value === 'code3' || 右アクティブタブ.value === 'code5') {
    return Web右アクティブコードRef.value?.接続状態表示 || '切断'
  }
  return イメージRef.value?.状態表示テキスト || '切断'
})

const Web右ペイン状態クラス = computed(() => {
  if (右アクティブタブ.value === 'code1' || 右アクティブタブ.value === 'code3' || 右アクティブタブ.value === 'code5') {
    return { on: Boolean(Web右アクティブコードRef.value?.WebSocket接続中), sending: false }
  }
  return {
    on: Boolean(イメージRef.value?.WebSocket接続中) && イメージRef.value?.接続状態 !== 'sending',
    sending: イメージRef.value?.接続状態 === 'sending',
...
function Web右ペイン対象(panel: PanelKey): panel is Extract<右ペインタブ型, PanelKey> {
  return panel === 'image' || panel === 'code1' || panel === 'code3' || panel === 'code5'
}

function コードAI名(channel: コードチャンネル): string {
  switch (channel) {
    ...
    case '5': return 表示モデル設定.value.CODE_AI5_NAME
    case '6': return 表示モデル設定.value.CODE_AI6_NAME
  }
}
...
const タスク編集ウィンドウ = computed(() => ウィンドウロール.value === 'taskDialog')
const チームウィンドウ = computed(() => ウィンドウロール.value === 'team')
const 設定SessionID = ref(new URLSearchParams(window.location.search).get('sessionId') || '')
const 設定キー = ref(0)
const タスク編集Payload = ref<Record<string, any> | null>(null)
...
const 現在タスクキー = computed<TaskKey | null>(() => {
  return TASK_KEYS.includes(ウィンドウロール.value as TaskKey) ? (ウィンドウロール.value as TaskKey) : null
})
...
const 現在コードチャンネル = computed<コードチャンネル | null>(() => {
...
const 現在コードモデル = computed(() => {
  switch (現在パネルキー.value) {
    ...
    case 'code5':
      return 表示モデル設定.value.CODE_AI5_NAME
    case 'code6':
      return 表示モデル設定.value.CODE_AI6_NAME
    default:
      return ''
  }
})
...
function 初期化処理(message: Record<string, any>) {
  ...
  const nextStates = {
    ...
    code5: Boolean(buttons.エージェント5),
    code6: Boolean(buttons.エージェント6),
  }
...
function ボタン状態取得() {
  return {
    ...
    エージェント5: パネル表示状態.value.code5,
    エージェント6: パネル表示状態.value.code6,
    チャットモード: チャットモード.value,
  }
}
...
async function パネル有効化(panel: PanelKey) {
  if (!isElectron) {
    if (Web右ペイン対象(panel)) {
      if (右アクティブタブ.value !== panel) {
        右アクティブタブ.value = panel
      }
    } else if (アクティブタブ.value !== panel) {
      アクティブタブ.value = panel
    }
    return
  }
...
// Electron: タイトルの TASK ボタンで AIタスク 3 ウィンドウ（要求 / フロー図 / 明細）を開閉する
function タスクウィンドウ切替() {
  void window.desktopApi?.toggleTaskWindows?.()
}
...
function タスク編集ウィンドウを閉じる() {
  void window.desktopApi?.closeCurrentWindow?.()
}

function タスク編集登録完了(item: Record<string, any> | null) {
  const channel = new BroadcastChannel('avatar-task-dialog')
  channel.postMessage({
    type: 'task-dialog-registered',
    kind: タスク編集Payload.value?.kind,
    item,
  })
  channel.close()
  タスク編集ウィンドウを閉じる()
...
function コードキャンセル処理(channel: コードチャンネル) {
  ウィンドウから入力ペイロード送信({
    チャンネル: channel,
    メッセージ識別: 'cancel_run',
    メッセージ内容: '強制停止！',
...
function コードファイル送信処理(payload: { channel: コードチャンネル; fileName: string; base64: string }) {
  ウィンドウから入力ペイロード送信({
...
onMounted(async () => {
  ...
  // AIタスクウィンドウは localStorage の認証情報だけで動く（コア接続は行わない）
  if (現在タスクキー.value) {
    認証読込中.value = false
    return
  }

  // AIチーム専用ウィンドウはAvatarのlocalStorage認証だけを使い、AIコア接続は開始しない
  if (チームウィンドウ.value) {
    ストレージ認証同期()
    if (認証トークン.value && !利用者.value) {
      try {
        await 現在利用者取得()
      } catch {
        認証クリア('認証状態を確認できませんでした。再ログインしてください。')
      }
    }
    認証読込中.value = false
    return
  ...
  if (タスク編集ウィンドウ.value) {
    認証読込中.value = false
    タスク編集Payload.value = await window.desktopApi?.getTaskDialogPayload?.() ?? null
    window.desktopApi?.onTaskDialogPayload?.((payload) => {
      タスク編集Payload.value = payload
    })
    return
  }
...
<template>
  <main class="app-root">
    ...
    <!-- ======= Web版 左右分割レイアウト（左上ボタンで AIタスク画面と切替） ======= -->
    <div v-else-if="コアウィンドウ && 認証済み && !isElectron" class="web-core-root">
    <div v-show="!AIタスク表示 && !AIチーム表示" class="web-split-layout" :class="{ resizing: リサイズ中 }">
      ...
      <div class="web-left-pane" :style="{ width: 分割比率 + '%' }">
        ...
        <div class="web-tab-content">
          ...
          <!-- コード6 -->
          <div v-show="アクティブタブ === 'code6'" class="web-tab-panel">
            <component
              :is="AIコアコード"
              :key="`${パネル再接続キー}-6`"
              ref="コード6Ref"
      ...
      <div class="web-right-pane" :style="{ width: (100 - 分割比率) + '%' }">
        <nav class="web-right-tab-bar">
          <button
            v-for="tab in webRightTabs"
            :key="tab.key"
            :class="['web-right-tab-btn', 右アクティブタブ === tab.key ? 'active' : '']"
            type="button"
            @click="右アクティブタブ = tab.key"
          >{{ tab.label }}</button>
          <span class="web-tab-spacer" />
        ...
        <div v-if="右アクティブタブ !== 'core'" class="web-tab-title">
          <div class="web-tab-title-left">
            <span>{{ Web右ペインタイトル }}</span>
          </div>
          <div class="web-tab-title-right">
            <span :class="['web-panel-status-dot', Web右ペイン状態クラス]"></span>
            <span class="web-panel-status-text">{{ Web右ペイン状態表示 }}</span>
          </div>
        </div>
        <div class="web-right-content">
          <div v-show="右アクティブタブ === 'core'" class="web-right-panel">
            <component
              :is="AIコアコントロール"
              ref="コアViewRef"
              :session-id="セッションID"
          ...
          <div v-show="右アクティブタブ === 'image'" class="web-right-panel">
            <component
              :is="AIコアイメージ"
              :key="パネル再接続キー"
              ref="イメージRef"
          ...
          <div v-show="右アクティブタブ === 'code1'" class="web-right-panel">
            <component
              :is="AIコアコード"
              :key="`${パネル再接続キー}-1`"
              ref="コード1Ref"
          ...
          <div v-show="右アクティブタブ === 'code3'" class="web-right-panel">
            <component
              :is="AIコアコード"
              :key="`${パネル再接続キー}-3`"
              ref="コード3Ref"
          ...
          <div v-show="右アクティブタブ === 'code5'" class="web-right-panel">
            <component
              :is="AIコアコード"
              :key="`${パネル再接続キー}-5`"
              ref="コード5Ref"
    ...
    <!-- Web版 AIタスク画面（要求 / フロー図 / 明細） -->
    <div v-if="AIタスク表示" class="web-task-layout">
      <div class="web-task-topbar">
        <button
          type="button"
          class="view-flip-btn"
          title="アバター画面へ切り替え"
          @click="AIタスク表示 = false"
        >🔄</button>
        <span class="web-task-title">AIタスク</span>
        <span class="web-tab-spacer" />
        <span class="web-right-user" :title="利用者ラベル">{{ 利用者ラベル }}</span>
      </div>
    ...
    <!-- Web版 AIチーム画面（frontend_avatar内へ移植した画面を同一ページで表示） -->
    <div v-if="AIチーム表示" class="web-team-layout">
      <div class="web-task-topbar">
        <button
          type="button"
          class="view-flip-btn"
          ...
          @click="AIチーム表示 = false"
        >🔄</button>
        <span class="web-task-title">AIチーム</span>
        <span class="web-tab-spacer" />
        <span class="web-right-user" :title="利用者ラベル">{{ 利用者ラベル }}</span>
      </div>
      <div class="web-team-body">
        <component
          :is="AIチームビュー"
          :利用者ID="String(利用者?.利用者ID || '')"
          :利用者名="String(利用者?.利用者名 || 利用者?.利用者ID || '')"
        />
      </div>
    </div>
    </div>

    <!-- ======= Electron コアウィンドウ（タイトルの TASK ボタンで AIタスク 3 ウィンドウを開く） ======= -->
    <component
      :is="AIコアコントロール"
    ...
    <!-- Electron AIチーム専用ウィンドウ -->
    <div v-else-if="チームウィンドウ && 認証済み && isElectron" class="team-window-root">
      <component
        :is="AIチームビュー"
        :利用者ID="String(利用者?.利用者ID || '')"
        :利用者名="String(利用者?.利用者名 || 利用者?.利用者ID || '')"
    ...
    <div v-else-if="タスク編集ウィンドウ" class="task-dialog-window-root">
      ...
      >
        <div v-if="!タスク編集Payload" class="task-dialog-loading">
          読み込み中...
        </div>
        <component
          :is="AIタスク明細編集"
        ...
        <component
          ...
          v-else
          :is-open="true"
          :key="`request-${タスク編集Payload?.編集タスク?.タスクID || 'new'}`"
          :利用者ID="String(タスク編集Payload?.利用者ID || '')"
          :編集タスク="タスク編集Payload?.編集タスク || null"
          @close="タスク編集ウィンドウを閉じる"
          @registered="タスク編集登録完了"
        />
      </component>
    ...
    <component
      ...
      :title="現在パネルキー ? PANEL_TITLES[現在パネルキー] : ''"
      theme="purple"
    >
      <template #title-right>
    ...
    <!-- ======= Electron AIタスクウィンドウ（task1: 要求 / task2: フロー図 / task3: 明細） ======= -->
    <component
      :is="WindowShell"
      v-else-if="現在タスクキー === 'task1'"
      title="AiDiy AI Task (タスク要求)"
      theme="purple"
    >
      <template #title-right>
        <button
        ...
        <button
          ...
          type="button"
          title="タスク要求を新規作成"
          @click="タスク要求WindowRef?.新規ダイアログ表示()"
        >新規</button>
      </template>
      <component
        :is="AIタスク要求ウィンドウ"
...
.task-dialog-window-root {
  width: 100%;
  height: 100%;
  background: #07080c;
}

.task-dialog-window-root :deep(.window-body) {
  background: #07080c;
}
...
.task-dialog-window-root :deep(.dialog-overlay) {
  position: static;
  inset: auto;
  background: transparent;
  z-index: auto;
...
.task-dialog-window-root :deep(.dialog-content) {
  width: 100%;
  max-width: none;
  height: 100%;
  border: none;
...
.task-dialog-window-root :deep(.dialog-header) {
  display: none;
}

.task-dialog-window-root :deep(.dialog-body) {
  flex: 1;
  overflow: auto;
}

.task-dialog-window-root :deep(button),
.task-dialog-window-root :deep(select),
.task-dialog-window-root :deep(input),
.task-dialog-window-root :deep(textarea) {
...
/* 画面切替ボタン（カメラの裏表切替風の丸ボタン） */
.view-flip-btn {
  width: 26px;
  height: 26px;
  padding: 0;
  margin-right: 4px;
  border: 1px solid rgba(150, 120, 240, 0.8);
  border-radius: 50%;
...
.web-right-tab-btn:hover {
  background: rgba(118, 97, 204, 0.25);
  color: #ddd;
}
.web-right-tab-btn.active {
  background: rgba(118, 97, 204, 0.45);
  color: #fff;
