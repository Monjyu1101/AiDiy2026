    ...
    <div v-else-if="コアウィンドウ && 認証済み && !isElectron" class="web-split-layout" :class="{ resizing: リサイズ中 }">
      ...
      <div class="web-left-pane" :style="{ width: 分割比率 + '%' }">
        <nav class="web-tab-bar">
          ...
          <span :class="['web-conn-dot', 入力接続済み ? 'on' : '']" :title="入力接続済み ? '接続中' : '切断'" />
          <span class="web-tab-user">{{ 利用者ラベル }}</span>
          <button class="web-tab-logout" type="button" @click="認証クリア()">ログアウト</button>
        </nav>

        <!-- タイトルバー -->
        <div class="web-tab-title">
          <span>{{ webTabs.find(t => t.key === アクティブタブ)?.label }}</span>
        </div>
...
.web-right-pane :deep(.window-shell) {
  width: 100% !important;
...
.web-tab-logout:hover { background: rgba(200,60,60,0.5); }
...
.web-tab-title {
  ...
  align-items: center;
  padding: 3px 10px;
  background: rgba(118, 97, 204, 0.15);
  border-bottom: 1px solid rgba(118, 97, 204, 0.3);
  font-size: 11px;
  font-weight: bold;
  color: #bba8ff;
  flex-shrink: 0;
