...
def 引数を解析する() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="frontend_avatar three-vrm viewer")
    ...
    def 状態を反映する(self) -> None:
        ...
        if (
            ...
            self.webview.page().runJavaScript(
                f"window.updateAvatarView && window.updateAvatarView({{zoom: {camera_zoom}, offsetX: {camera_offset_x}, offsetY: {camera_offset_y}}});"
            )
        if camera_command and camera_command_seq != last_camera_command_seq:
            self.webview.page().runJavaScript(
                f"window.applyAvatarCommand && window.applyAvatarCommand({json.dumps(camera_command, ensure_ascii=False)});"
            )
...
def main() -> int:
    ...
    index_url = f"http://127.0.0.1:{server.port}/アバター制御/three_vrm/three_vrm_index.html?model={state['model_url']}"
    window = AvatarWindow(args.state_file, index_url, args.parent_pid)
    window.show()
    try:
        return app.exec()
    finally:
